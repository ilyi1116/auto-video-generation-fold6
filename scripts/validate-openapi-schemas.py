...
class OpenAPIValidator:
    ...
    async def fetch_openapi_spec(self, session: aiohttp.ClientSession, 
                                service_name: str, base_url: str) -> Optional[Dict]:
        """獲取服務的 OpenAPI 規範"""
        openapi_urls = [
    ...
    def validate_openapi_structure(self, service_name: str, spec: Dict) -> None:
        ...
        if "openapi" in spec:
            ...
            if not version.startswith("3."):
                self.warnings.append(f"{service_name}: Using OpenAPI version {version}, recommend 3.x")
        ...
        for path, methods in paths.items():
            for method, operation in methods.items():
                ...
                if "responses" not in operation:
                    self.errors.append(f"{service_name}: No responses defined for {method.upper()} {path}")
                    continue
                ...
                if not success_codes:
                    self.warnings.append(f"{service_name}: No success response for {method.upper()} {path}")
                
                # 檢查是否有錯誤處理
                error_codes = [code for code in responses.keys() if code.startswith(("4", "5"))]
                if not error_codes:
                    self.warnings.append(f"{service_name}: No error responses for {method.upper()} {path}")
        ...
        for service_name, spec in specs.items():
            if "components" in spec and "securitySchemes" in spec["components"]:
                ...
                for scheme_name, scheme_def in schemes.items():
                    key = (scheme_name, scheme_def.get("type"), scheme_def.get("scheme"))
