...
class NetworkConnectivityTester:
    ...
    async def _test_tcp_connection(
        self, host: str, port: int, timeout: int = None
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if health_response.status_code == 200:
                    ...
                    return (
                        False,
                        response_time,
                        f"API 健康检查失败: HTTP {health_response.status_code}",
                        {
    ...
    async def test_single_target(
        ...
        try:
            ...
            return ConnectivityResult(
                ...
                target=config.get(
                    "url",
                    f"{config.get('host', 'unknown')}:{config.get('port', 'unknown')}",
                ),
                status=status,
    ...
    async def test_service_to_service_connectivity(
        self,
    ) -> List[ConnectivityResult]:
        """测试服务间连通性生成的）"""
