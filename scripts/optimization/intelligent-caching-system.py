...
class IntelligentCache:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None, 
                 tags: List[str] = None, force_level: Optional[CacheLevel] = None) -> bool:
    ...
    async def invalidate_by_tags(self, tags: List[str]) -> int:
        ...
        try:
            ...
            def invalidate_disk():
                ...
                count = 0
                for tag in tags:
                    cursor.execute("SELECT key, file_path FROM cache_entries WHERE tags LIKE ?", (f"%{tag}%",))
    ...
    async def _set_l1(self, key: str, value: Any, ttl: Optional[int] = None, 
                     tags: List[str] = None, promote: bool = False) -> bool:
        ...
        try:
            ...
            with self.l1_lock:
                # Evict if necessary
                while (len(self.l1_cache) >= self.l1_max_entries or 
                       self._get_l1_memory_usage() + entry_size > self.l1_max_size):
        ...
        try:
            ...
            if tags:
                for tag in tags:
                    ...
                    await asyncio.get_event_loop().run_in_executor(
                        None, lambda: self.redis_client.setex(tag_key, ttl, "1")
                    )
        ...
        for pattern in hot_patterns:
            if len(pattern.access_times) >= 3:
                ...
                if intervals:
                    ...
                    pattern.prediction_score = max(0, 1.0 - (std_dev / avg_interval)) if avg_interval > 0 else 0
    ...
    async def optimize_cache_configuration(self) -> Dict[str, Any]:
        ...
        if system_memory_percent > 90:
            recommendations.append({
                ...
                'actions': ['Increase compression threshold', 'Reduce L1 cache size', 'More aggressive eviction']
            })
        ...
        return {
            ...
            'performance_summary': {
                'overall_hit_rate': sum(s['hits'] for s in stats['levels'].values()) / 
                                  max(1, sum(s['hits'] + s['misses'] for s in stats['levels'].values())) * 100,
                'avg_access_time_ms': statistics.mean([s['avg_access_time_ms'] for s in stats['levels'].values()]),
...
class CacheManager:
    ...
    async def cached_function(self, func: Callable, *args, ttl: Optional[int] = None, 
                            tags: List[str] = None, **kwargs) -> Any:
