...
class ConfigurationValidator:
    ...
    def __init__(self, project_path: str = "."):
        ...
        logger.info(
            f"配置验证器初始化完成 - Termux: {self.is_termux}, 项目路径: {self.project_path}"
        )
    ...
    def _generate_text_report(self, report: Dict) -> str:
        ...
        for category, stats in report["category_statistics"].items():
            ...
            lines.append(
                f"  {category}: {total} (严重: {stats['critical']}, 警告: {stats['warning']}, 提示: {stats['info']})"
            )
...
def main():
    ...
    print(
        f"   总问题数: {summary['total_issues']} (严重: {summary['critical_issues']}, 警告: {summary['warning_issues']})"
    )
