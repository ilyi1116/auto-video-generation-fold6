...
class ResponseCache:
    ...
    def _generate_cache_key(self, method: str, url: str, params: Dict = None, 
        ...
        cache_data = {
            ...
            'url': url,
            'params': params or {},
            'cache_headers': {k: v for k, v in (headers or {}).items() 
...
class MicroservicesCommunicationOptimizer:
    ...
    async def make_request(self, from_service: str, to_service: str, method: str, 
                          endpoint: str, **kwargs) -> Tuple[Optional[Dict], CommunicationMetrics]:
        ...
        try:
            ...
            async with session.request(method, url, **request_data, **kwargs) as response:
                ...
                ) if instance.avg_response_time else response_time * 1000
                
                # Cache successful GET responses
                if (method.upper() == 'GET' and response.status == 200 and 
                    self.response_cache and cache_key):
                    await self.response_cache.set(cache_key, {
                        'data': response_data,
                ...
                try:
                    ...
                    async with session.get(
                        ...
                        if is_healthy:
                            ...
                            ) if instance.avg_response_time else response_time
                    ...
                    instance.last_health_check = datetime.now() 
                    logger.warning(f"Health check failed for {instance.name} at {instance.host}:{instance.port}: {e}")
    ...
    async def optimize_communication_patterns(self):
        ...
        for service_name, instances in self.services.items():
            ...
            avg_response_time = statistics.mean([inst.avg_response_time or 0 for inst in healthy_instances])
    ...
    async def start_monitoring(self, monitoring_interval: int = 30):
        ...
        logger.info(f"Starting communication optimizer monitoring (interval: {monitoring_interval}s)")
        ...
        while True:
            try:
                ...
                logger.info(f"Health check completed - {total_healthy}/{total_instances} instances healthy")
                ...
                if self.response_cache:
                    cache_stats = self.response_cache.get_cache_stats()
                    logger.info(f"Cache performance - Hit rate: {cache_stats['hit_rate']:.1f}%, Total requests: {cache_stats['total_requests']}")
...
async def main():
    ...
    parser.add_argument('--config', default='config/communication-optimizer.yaml', help='Configuration file')
