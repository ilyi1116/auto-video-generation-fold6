...
async def main():
    ...
    for service_name in ["api-gateway", "auth-service", "data-service"]:
        ...
        print(f"   {service_name}: {stats['healthy_instances']}/{stats['total_instances']} 健康實例")
    ...
    for strategy, strategy_name in strategies:
        ...
        # 註冊測試服務
        await temp_discovery.register_service("test-service", "host1", 8001, weight=1)
        await temp_discovery.register_service("test-service", "host2", 8002, weight=2)
        await temp_discovery.register_service("test-service", "host3", 8003, weight=3)
        
        # 獲取實例並顯示選擇結果
        selected_hosts = []
        for _ in range(6):
            instance = await temp_discovery.get_service_instance("test-service")
            if instance:
