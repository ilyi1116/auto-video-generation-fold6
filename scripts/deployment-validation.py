...
class DeploymentValidator:
    ...
    def __init__(self, project_path: str = "."):
        ...
        # 基础设施端点
        self.infrastructure = [
            ServiceEndpoint(name="postgres", url="postgresql://localhost:5432", health_path="", timeout=10),
            ServiceEndpoint(name="redis", url="redis://localhost:6379", health_path="", timeout=10),
            ServiceEndpoint(name="minio", url="http://localhost:9000", health_path="/minio/health/live"),
        ]
    ...
    async def test_docker_compose_deployment(self) -> bool:
        ...
        if not self.docker_available:
            self._add_result(
                ...
                False,
                "Docker 不可用，跳过部署测试",
                details={"reason": "docker_unavailable", "environment": "termux"},
                duration=time.time() - start_time
            )
            return False
            ...
            if service.name == "postgres":
                ...
                async with httpx.AsyncClient(timeout=service.timeout) as client:
                    ...
                    self._add_result(
                        f"Service Health - {service.name}",
                        success,
                        f"服务响应正常 ({response.status_code})" if success else f"服务异常 ({response.status_code})",
                        details=details,
                        duration=time.time() - start_time
                    )
                    return success
    ...
    async def run_all_validations(self) -> Dict:
        ...
        logger.info(f"✅ 验证完成 - {passed_tests}/{total_tests} 测试通过 ({summary['summary']['pass_rate']})")
        
        return summary
    
    def save_report(self, summary: Dict, filename: str = "deployment-validation-report.json"):
        """保存验证报告"""
...
async def main():
    ...
    parser.add_argument("--output", "-o", default="deployment-validation-report", help="报告文件名（不含扩展名）")
