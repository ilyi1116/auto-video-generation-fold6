...
class GDPRComplianceManager:
    ...
    async def record_consent(self, consent: ConsentRecord) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Consent recorded for user {consent.user_id}, purpose: {consent.purpose.value}"
            )
            return True
...
class DataRequestHandler:
    ...
    async def create_data_request(
        self, user_id: int, request_type: str, details: Dict = None
    ) -> str:
        """創建資料請求"""
        try:
            # 生成驗證令牌
    ...
    async def _get_user_consent_history(self, user_id: int) -> List[Dict]:
        ...
        consents = (
            self.db.query(GDPRConsentLog)
            .filter(GDPRConsentLog.user_id == user_id)
            .order_by(GDPRConsentLog.timestamp.desc())
            .all()
        )
    ...
    async def _get_user_processing_logs(self, user_id: int) -> List[Dict]:
        """獲取用戶資料處理日誌"""
        logs = (
            self.db.query(GDPRDataProcessingLog)
            .filter(GDPRDataProcessingLog.user_id == user_id)
            .order_by(GDPRDataProcessingLog.timestamp.desc())
            .limit(100)
            .all()
        )
