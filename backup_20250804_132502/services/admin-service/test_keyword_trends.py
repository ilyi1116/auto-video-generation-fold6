...
from .models import Base, KeywordTrend, SocialTrendConfig, SocialPlatform
from .database import get_db
from .main import app
from .crud import crud_keyword_trend
from .social_trends import collect_and_save_trends, SocialTrendsService
from .tasks.trends_tasks import collect_keyword_trends_new
...
class TestKeywordTrendCRUD:
    ...
    def test_get_latest_trends(self, db_session, sample_keyword_trends):
        ...
        # 獲取最新趨勢
        latest_trends = crud_keyword_trend.get_latest_trends(
            db_session, period="day", limit=10
        )
    ...
    def test_search_keywords(self, db_session, sample_keyword_trends):
        ...
        # 搜尋包含 "創作" 的關鍵字
        results = crud_keyword_trend.search_keywords(
            db_session, search_term="創作"
        )
...
class TestSocialTrendsService:
    ...
    async def test_collect_and_save_trends(self):
        ...
        with patch('services.admin-service.social_trends.social_trends_service') as mock_service:
            ...
            result = await collect_and_save_trends(
                platforms=["TikTok"], 
                period="day"
            )
        ...
        with patch('services.admin-service.main.get_current_user') as mock_auth:
            ...
            with patch('services.admin-service.main.collect_and_save_trends') as mock_collect:
                mock_collect.return_value = {
                    "success": True,
                    "total_records_saved": 50
                }
                
                response = client.post("/admin/keyword-trends/collect", 
                                     json={"platforms": ["TikTok"], "period": "day"})
...
class TestDataIntegrity:
    ...
    def test_duplicate_prevention(self, db_session):
        ...
        # 檢查兩條記錄都存在（因為時間不同）
        count = db_session.query(KeywordTrend).filter(
            KeywordTrend.keyword == "重複測試"
        ).count()
        assert count == 2
...
class TestPerformance:
    ...
    def test_large_dataset_query(self, db_session):
        ...
        results = crud_keyword_trend.get_latest_trends(
            db_session, limit=50
        )
        end_time = time.time()
