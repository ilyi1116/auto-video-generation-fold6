...
class BehaviorAnalyzer:
    ...
    async def analyze_behavior(self, user_id: str, action: str, context: SecurityContext) -> float:
        ...
        # 綜合風險評估
        total_risk = (time_risk * 0.2 + location_risk * 0.3 + 
                     frequency_risk * 0.3 + device_risk * 0.2)
    ...
    async def _analyze_location_patterns(self, user_id: str, location: Optional[Dict[str, str]]) -> float:
        """分析地理位置異常"""
        if not location:
            return 0.3
    ...
    async def _record_behavior(self, user_id: str, action: str, context: SecurityContext, risk_score: float):
        """記錄用戶行為"""
        behavior_record = {
            'action': action,
...
class RiskEngine:
    ...
    async def assess_risk(self, user_id: str, action: str, context: SecurityContext) -> float:
        ...
        logger.info(f"風險評估 - 用戶:{user_id}, 行為:{behavior_risk:.3f}, "
...
async def main():
    ...
    if result["authorized"]:
        print(f"✅ 訪問授權 - 信任等級: {result['trust_level']}, 風險分數: {result['risk_score']:.3f}")
    else:
