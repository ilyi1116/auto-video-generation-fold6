...
class SecurityScanner:
    ...
    async def _run_bandit_scan(self, target_path: str) -> List[SecurityFinding]:
        ...
        try:
            ...
            cmd = [
                'docker', 'run', '--rm',
                '-v', f'{os.path.abspath(target_path)}:/code',
                '-v', f'{self.config_dir}:/config',
                'securityanalysis/bandit',
                '-r', '/code',
                '-f', 'json',
                '-c', '/config/bandit-config.yaml'
            ]
    ...
    async def _run_safety_scan(self, target_path: str) -> List[SecurityFinding]:
        ...
        for req_file in req_files:
            try:
                cmd = [
                    'docker', 'run', '--rm',
                    '-v', f'{req_file.parent}:/code',
                    'pyupio/safety',
                    'check', '--json', '-r', f'/code/{req_file.name}'
                ]
    ...
    async def _run_gitleaks_scan(self, target_path: str) -> List[SecurityFinding]:
        ...
        try:
            cmd = [
                ...
                'detect', '--source=/code', '--report-format=json', '--report-path=/tmp/gitleaks.json'
            ]
    ...
    async def _scan_container_image(self, image_name: str) -> ScanResult:
        ...
        try:
            cmd = [
                ...
                '-v', f'{self.config_dir}:/config',
                'aquasec/trivy',
                'image', '--config', '/config/trivy-config.yaml',
                '--format', 'json',
                image_name
            ]
    ...
    async def _generate_comprehensive_report(self, results: Dict[str, ScanResult]) -> Dict[str, Any]:
        """生成綜合報告"""
        report = {
