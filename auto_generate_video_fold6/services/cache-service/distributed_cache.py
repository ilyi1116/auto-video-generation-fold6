...
class MemoryCache:
    ...
    def _evict(self):
        ...
        if self.eviction_policy == EvictionPolicy.LRU:
            ...
            # 淘汰即將過期的
            now = datetime.utcnow()
            min_ttl_key = min(self.cache.keys(),
...
class RedisClusterCache:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None, 
                  compress: bool = False, serialize: bool = True):
        """設置快取值"""
        try:
            # 序列化複雜對象
    ...
    async def record_access(self, key: str, context: Dict[str, Any] = None):
        ...
        if len(self.access_patterns[key]['access_times']) > 100:
            self.access_patterns[key]['access_times'] = \
                self.access_patterns[key]['access_times'][-100:]
            self.access_patterns[key]['contexts'] = \
                self.access_patterns[key]['contexts'][-100:]
...
class CachePerformanceMonitor:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            hit_ratio = (self.metrics['total_hits'] / self.metrics['total_gets'] 
                        if self.metrics['total_gets'] > 0 else 0)
            
            avg_get_time = (np.mean(self.metrics['get_times']) 
                           if self.metrics['get_times'] else 0)
            avg_set_time = (np.mean(self.metrics['set_times']) 
                           if self.metrics['set_times'] else 0)
            ...
            return {
                ...
                'p95_get_time_ms': (np.percentile(self.metrics['get_times'], 95) * 1000 
                                   if self.metrics['get_times'] else 0),
                'p99_get_time_ms': (np.percentile(self.metrics['get_times'], 99) * 1000 
                                   if self.metrics['get_times'] else 0)
            }
