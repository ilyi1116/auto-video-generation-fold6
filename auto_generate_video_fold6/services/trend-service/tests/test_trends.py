...
def test_get_trending_topics_with_filters(mock_verify_token, client, auth_headers):
    ...
    response = client.get(
        "/api/v1/trends/trending?platform=youtube&category=tech&limit=10", headers=auth_headers
    )
...
def test_get_viral_content_with_filters(mock_verify_token, client, auth_headers):
    ...
    response = client.get(
        "/api/v1/trends/viral?platform=tiktok&days=3&min_viral_score=80", headers=auth_headers
    )
...
async def test_get_keyword_performance(mock_verify_token, mock_performance, client, auth_headers):
    ...
    response = client.get(
        "/api/v1/trends/performance/AI?platforms=google&days=30", headers=auth_headers
    )
...
async def test_research_keyword(mock_verify_token, mock_analyze, client, auth_headers):
    ...
    research_data = {"keyword": "AI", "platforms": ["google", "youtube"], "region": "TW"}
...
def test_invalid_platform_enum(client, auth_headers):
    ...
    with patch("app.auth.verify_token") as mock_verify_token:
        ...
        response = client.get(
            "/api/v1/trends/trending?platform=invalid_platform", headers=auth_headers
        )
