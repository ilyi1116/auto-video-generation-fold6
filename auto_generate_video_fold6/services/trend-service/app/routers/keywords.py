...
async def research_keyword(
    ...
    try:
        ...
        research_data = await keyword_analyzer.analyze_keyword(
            keyword=request.keyword, platforms=request.platforms, region=request.region
        )
...
async def get_keyword_difficulty(
    keyword: str, platform: str = Query("google"), current_user: dict = Depends(verify_token)
):
    """分析關鍵字競爭難度"""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Keyword difficulty analysis failed: {str(e)}")
...
async def get_keyword_volume(
    ...
    try:
        volume_data = await keyword_analyzer.get_search_volume_trends(
            keyword=keyword, platforms=platform_list, region=region, timeframe=timeframe
        )
...
async def compare_keywords(
    ...
    if len(keyword_list) > 10:
        raise HTTPException(status_code=400, detail="Maximum 10 keywords allowed for comparison")
...
async def find_keyword_gaps(
    ...
    try:
        gap_analysis = await keyword_analyzer.find_keyword_gaps(
            competitor_domain=competitor_domain, your_domain=your_domain, limit=limit
        )
...
async def get_content_ideas(
    ...
    try:
        content_ideas = await keyword_analyzer.generate_content_ideas(
            keyword=keyword, content_type=content_type, platform=platform, limit=limit
        )
