...
async def get_trending_hashtags(platform: str, category: str = None, limit: int = 30) -> List[Dict[str, Any]]:
    """獲取熱門標籤"""
...
async def get_keyword_performance_history(
    ...
    return {
        ...
        "summary": {
            "average_volume": sum(d["volume"] for data in platform_data.values() for d in data) // (len(platforms) * days),
