...
class AIOrchestrator:
    ...
    def _get_available_providers(self, task_type: AITaskType) -> List[AIProvider]:
        ...
        providers_map = {
            ...
            AITaskType.CONTENT_ANALYSIS: [AIProvider.GEMINI, AIProvider.OPENAI],
    ...
    async def _execute_text_generation(self, request: AIRequest, provider: AIProvider, start_time: float) -> AIResponse:
        """執行文字生成"""
    ...
    async def _execute_music_generation(self, request: AIRequest, provider: AIProvider, start_time: float) -> AIResponse:
        """執行音樂生成"""
    ...
    async def _execute_content_analysis(self, request: AIRequest, provider: AIProvider, start_time: float) -> AIResponse:
        """執行內容分析"""
        if provider == AIProvider.GEMINI and GEMINI_AVAILABLE:
            # 使用 Gemini 進行內容分析
            analysis_prompt = f"""
...
"""
            ...
            async with GeminiClient(api_key=api_key) as client:
                ...
                if result.success:
                    try:
                        ...
                        analysis_data = json.loads(json_match.group()) if json_match else {"raw_text": result.text}
    ...
    async def _execute_trend_analysis(self, request: AIRequest, provider: AIProvider, start_time: float) -> AIResponse:
        """執行趨勢分析"""
        if provider == AIProvider.GEMINI and GEMINI_AVAILABLE:
    ...
    async def _try_fallback(self, request: AIRequest, failed_provider: AIProvider) -> Optional[AIResponse]:
        """嘗試故障轉移"""
        logger.info(f"嘗試故障轉移，原提供商: {failed_provider.value}")
        
        # 獲取其他可用提供商
        available_providers = self._get_available_providers(request.task_type)
        fallback_providers = [p for p in available_providers if p != failed_provider and self.provider_health.get(p, False)]
    ...
    async def _update_provider_metrics(self, provider: AIProvider, success: bool, duration: float):
        ...
        metrics["average_response_time"] = (
            (metrics["average_response_time"] * (total_requests - 1) + duration) / total_requests
        )
