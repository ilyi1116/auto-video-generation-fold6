...
class AIManager:
    ...
    async def _update_health_status(self):
        ...
        self.service_health = {
            ...
            "image_generation": self.image_generator.is_healthy() if self.image_generator else False,
            "audio_processing": self.audio_processor.is_healthy() if self.audio_processor else False,
        ...
        try:
            ...
            if media_tasks:
                task_results = await asyncio.gather(
                    *[task[1] for task in media_tasks],
                ...
                for i, (media_type, _) in enumerate(media_tasks):
                    ...
                    if not isinstance(result, Exception):
                        ...
                        logger.warning(f"Failed to generate {media_type}", error=str(result))
    ...
    def get_service_capabilities(self) -> Dict[str, Any]:
        ...
        capabilities = {
            "text_generation": {
                ...
                "features": ["script_generation", "title_generation", "content_optimization"] if self.text_generator else [],
                "supported_languages": ["en", "zh", "ja", "ko", "es", "fr", "de"] if self.text_generator else []
            },
            "image_generation": {
                "available": self.service_health["image_generation"],
                "features": ["image_generation", "style_transfer", "upscaling", "variations"] if self.image_generator else [],
                "supported_styles": ["photorealistic", "artistic", "cartoon", "anime"] if self.image_generator else []
            },
            "audio_processing": {
                "available": self.service_health["audio_processing"],
                "features": ["voice_synthesis", "voice_cloning", "audio_enhancement"] if self.audio_processor else [],
                "supported_voices": ["natural", "professional", "energetic"] if self.audio_processor else []
            },
            "music_generation": {
                "available": self.service_health["music_generation"],
                "features": ["music_generation", "variations", "extension"] if self.suno_client else [],
                "supported_styles": ["background", "energetic", "cinematic"] if self.suno_client else []
            }
        }
