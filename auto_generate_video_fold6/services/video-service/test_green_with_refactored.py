...
# 使用重構後的實作
from workflow_engine_refactored import VideoWorkflowEngine, VideoWorkflowRequest, VideoWorkflowResult
...
def test_should_execute_complete_workflow_pipeline():
    ...
    pipeline_result = executor.execute_pipeline(
        ...
        stages=["script_generation", "image_creation", "voice_synthesis", "video_composition"]
    )
...
def main():
    ...
    # 原始 Green 測試列表
    tests = [
        (test_should_create_workflow_request_with_valid_data, "創建工作流程請求"),
        (test_should_initialize_video_workflow_successfully, "初始化影片工作流程"),
        (test_should_execute_complete_workflow_pipeline, "執行完整工作流程管道"),
        (test_should_handle_workflow_stage_failures_gracefully, "處理工作流程階段失敗"),
