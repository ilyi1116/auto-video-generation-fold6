...
class VideoGenerationService:
    ...
    async def _generate_video_async(
        ...
        try:
            ...
            script_data = await self._generate_script(request.topic, request.target_platform, request.video_length)
            ...
            voice_url = await self._generate_voice(script_data["full_script"], request.voice_settings)
            ...
            final_result = await self.composer.render_final(
                composition_result.composition_id,
                request.quality
            )
            
            # Step 8: Complete
            await self._update_status(
                generation_id, "completed", 100, "Video generation completed",
                {
