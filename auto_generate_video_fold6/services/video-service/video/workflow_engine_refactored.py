...
class VideoWorkflowRequest:
    ...
    def __init__(self, topic: str, target_platform: str = "youtube", 
        ...
        self._validate_inputs(topic, target_platform, workflow_type, quality_level, expected_duration)
    ...
    def _validate_inputs(self, topic: str, target_platform: str, workflow_type: str, 
                        quality_level: str, expected_duration: int) -> None:
        """驗證輸入參數"""
        ...
        if workflow_type not in valid_workflow_types:
            raise ValueError(f"Invalid workflow type. Must be one of: {valid_workflow_types}")
        
        valid_quality_levels = {"low", "medium", "high", "ultra"}
        if quality_level not in valid_quality_levels:
            raise ValueError(f"Invalid quality level. Must be one of: {valid_quality_levels}")
...
class VideoWorkflowResult:
    ...
    def __init__(self, workflow_id: str, status: WorkflowStatus, current_stage: WorkflowStage,
    ...
    def update_progress(self, stage: WorkflowStage, progress: int, 
...
class WorkflowRepository(Protocol):
    ...
    def save(self, workflow_id: str, request: VideoWorkflowRequest, 
             result: VideoWorkflowResult, user_id: str) -> None:
        """儲存工作流程"""
        pass
...
class CompletionTimeEstimator:
    ...
    def _get_duration_multiplier(self, duration: int) -> float:
        """取得長度乘數"""
        for (min_dur, max_dur), multiplier in self._duration_multipliers.items():
    ...
    def _execute_stages(self, result: VideoWorkflowResult) -> None:
        ...
        for stage, progress in stages:
            result.update_progress(
                stage=stage,
                progress=progress,
                assets=self._generate_mock_assets(result.workflow_id) if progress == 100 else None
            )
