...
class WorkflowConfigRequest(BaseModel):
    ...
    target_categories: List[str] = Field(default=["technology", "entertainment", "lifestyle"], description="目標內容類別")
    target_platforms: List[str] = Field(default=["tiktok", "youtube-shorts"], description="目標平台")
    ...
    content_quality_threshold: float = Field(default=0.8, ge=0.1, le=1.0, description="內容品質門檻")
    ...
    cost_per_video_limit: float = Field(default=5.0, ge=0.5, le=50.0, description="單部影片成本限制（USD）")
    stop_on_budget_exceeded: bool = Field(default=True, description="超出預算時是否停止")
    operating_hours: Dict[str, str] = Field(default={"start": "09:00", "end": "18:00"}, description="運行時間")
    max_workflow_duration: int = Field(default=1800, ge=300, le=7200, description="最大工作流程時間（秒）")
    ...
    optimal_publishing_times: List[str] = Field(default=["10:00", "14:00", "18:00"], description="最佳發布時間")
...
async def get_workflow_result(
    ...
    try:
        ...
        metrics_dict = {
            ...
            "total_duration": str(result.metrics.total_duration) if result.metrics.total_duration else None,
