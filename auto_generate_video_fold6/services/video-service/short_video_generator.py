...
class ShortVideoGenerator:
    ...
    async def _create_text_overlay(self, keyword: str, work_dir: Path) -> str:
        ...
        try:
            ...
            # 標題位置
            title_y = 200
            draw.text((width//2, title_y), title_text, font=title_font, 
                     fill=(255, 255, 255, 255), anchor="mm", stroke_width=3, stroke_fill=(0, 0, 0, 255))
            
            # 副標題
            subtitle_text = "#熱門 #趨勢 #必看"
            subtitle_y = title_y + 120
            draw.text((width//2, subtitle_y), subtitle_text, font=subtitle_font, 
                     fill=(255, 200, 0, 255), anchor="mm", stroke_width=2, stroke_fill=(0, 0, 0, 255))
            
            # 底部 CTA
            cta_text = "👆 點擊了解更多"
            cta_y = height - 300
            draw.text((width//2, cta_y), cta_text, font=subtitle_font, 
                     fill=(255, 255, 255, 255), anchor="mm", stroke_width=2, stroke_fill=(0, 0, 0, 255))
    ...
    async def _create_video_scenes(self, visual_assets: Dict, script: str, category: str) -> List[Dict]:
        """創建影片場景"""
        scenes = []
        
        try:
            duration_per_scene = self.video_templates[category]['duration'] / len(visual_assets['background_images'])
    ...
    async def _assemble_video(self, scenes: List[Dict], audio_file: str, work_dir: Path, category: str) -> str:
        """組裝影片"""
        try:
            clips = []
    ...
    async def _add_tiktok_effects(self, video_path: str, keyword: str, work_dir: Path) -> str:
        ...
        try:
            ...
            # 2. 添加結尾 CTA
            outro_text = TextClip("關注獲取更多熱門內容 👆", fontsize=80, color='yellow', font='Arial')
            ...
            # 4. 添加趨勢標籤
            hashtag_text = TextClip(f"#{keyword} #熱門 #趨勢", fontsize=60, color='white', font='Arial')
            hashtag_text = hashtag_text.set_position(('left', 'bottom')).set_duration(final_video.duration)
