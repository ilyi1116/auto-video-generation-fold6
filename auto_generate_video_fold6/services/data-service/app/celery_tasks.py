...
@app.task(bind=True)
def start_preprocessing_task(self, job_id: int, file_id: int, s3_key: str, preprocessing_params: Dict[str, Any]):
    ...
    try:
        ...
        processed_s3_key = s3_key.replace(".wav", "_processed.wav").replace(".mp3", "_processed.wav")
        processed_s3_key = processed_s3_key.replace(os.path.splitext(processed_s3_key)[1], "_processed.wav")
...
async def update_job_status(job_id: int, status: str, progress: int, result_data: Dict[str, Any] = None, error_message: str = None):
    ...
    try:
        ...
        query = processing_jobs.update().where(
