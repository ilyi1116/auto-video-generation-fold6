...
async def test_scheduler_basic_functionality():
    ...
    # 測試預算限制
    scheduler.daily_stats["budget_used"] = 50.0  # 達到預算限制
    try:
        await scheduler.schedule_entrepreneur_task(task_config)
        assert False, "應該拋出預算限制異常"
    except ValueError as e:
        assert "已達每日預算限制" in str(e)
        print("✅ 預算限制測試通過")
...
async def test_scheduler_task_execution():
    ...
    result = {"success": True, "videos_generated": 1}
...
async def test_scheduler_statistics():
    ...
    assert scheduler.daily_stats["budget_used"] == 7.5
    ...
    assert scheduler.daily_stats["budget_used"] == 10.5
    print("✅ 累積統計測試通過")
    ...
    assert status["daily_stats"]["videos_generated"] == 5
    print("✅ 狀態獲取測試通過")
...
async def test_config_validation():
    ...
    # 測試無效時間格式
    try:
        invalid_config = SchedulerConfig(work_hours_start="25:00")  # 無效時間
