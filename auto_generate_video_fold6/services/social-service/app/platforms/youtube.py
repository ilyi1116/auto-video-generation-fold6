...
async def publish_video(
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"YouTube publish failed: {str(e)}"
        }
...
async def check_api_status() -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{settings.YOUTUBE_API_BASE}/videos?part=id&chart=mostPopular&maxResults=1") as response:
...
async def _get_video_file_url(video_id: int) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{settings.VIDEO_SERVICE_URL}/api/v1/videos/{video_id}/file") as response:
            if response.status == 200:
                result = await response.json()
