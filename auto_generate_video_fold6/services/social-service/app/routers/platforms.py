...
from ..schemas import (
    PublishRequest, PublishResponse, PlatformAuth, 
    OAuthCallback, TokenResponse
)
...
async def publish_to_tiktok(request: PublishRequest):
    ...
    try:
        ...
        return PublishResponse(
            success=False,
            error=f"Failed to publish to TikTok: {str(e)}"
        )
...
async def publish_to_youtube(request: PublishRequest):
    ...
    try:
        ...
        return PublishResponse(
            success=False,
            error=f"Failed to publish to YouTube: {str(e)}"
        )
