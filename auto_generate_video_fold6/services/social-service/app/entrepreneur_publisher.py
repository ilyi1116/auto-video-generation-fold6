...
class EntrepreneurPublisher:
    ...
    def _initialize_platform_configs(self) -> Dict[Platform, ContentOptimization]:
        """初始化平台配置"""
    ...
    async def schedule_batch_publish(
        self, 
        request: PublishRequest, 
        publish_times: Dict[Platform, datetime]
    ) -> str:
        """排程批次發布"""
        
        try:
            request_id = f"scheduled_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{request.user_id}"
            ...
            for platform, scheduled_time in publish_times.items():
                if platform in request.platforms:
                    ...
                    if delay_seconds > 0:
                        # 創建延遲任務
                        asyncio.create_task(
                            self._delayed_publish(request, platform, delay_seconds)
                        )
                        
                        logger.info(
                            f"已排程 {platform.value} 發布: {scheduled_time.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
    ...
    async def _delayed_publish(
        ...
        try:
            ...
            # 執行發布
            result = await self.publish_to_multiple_platforms(single_platform_request)
