...
class GDPRCompliance:
    ...
    async def validate_consent(self, consent_record: ConsentRecord) -> bool:
        ...
        if not is_valid:
            logger.warning(f"同意驗證失敗: {consent_record.consent_id}, 失敗項目: {[k for k, v in validations.items() if not v]}")
    ...
    async def process_erasure_request(self, subject_id: str, reason: str = "withdrawal_of_consent") -> bool:
    ...
    async def _store_audit_event(self, event: AuditEvent):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                """, (
                    event.event_id, event.event_type, event.user_id, event.resource,
                    event.action, event.timestamp, event.ip_address, event.user_agent,
                    event.success, event.risk_level, json.dumps(event.compliance_tags),
...
class ComplianceReportGenerator:
    ...
    async def generate_gdpr_report(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """生成 GDPR 合規報告"""
    ...
    async def _get_processing_activity_stats(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """獲取處理活動統計"""
        return {
            'total_activities': 0,
    ...
    async def _get_subject_rights_stats(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """獲取資料主體權利統計"""
        return {
            'total_requests': 0,
            'requests_by_type': {},
    ...
    async def _get_breach_events(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """獲取資料外洩事件"""
        return []
...
class ComplianceFramework:
    ...
    async def validate_data_processing(
        ...
        await self.audit_logger.log_compliance_event(
            ...
            "validate",
            self.enabled_standards,
            {"compliant": validation_results["compliant"], "activity": asdict(activity)}
        )
