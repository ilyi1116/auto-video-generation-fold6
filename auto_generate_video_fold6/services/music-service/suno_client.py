...
class SunoClient:
    ...
    async def _poll_generation_status(self, generation_id: str, start_time: float) -> MusicGenerationResult:
        """輪詢生成狀態"""
        ...
        while attempt < max_attempts:
            try:
                async with self.session.get(f"{self.base_url}/api/get?ids={generation_id}") as response:
                    if response.status == 200:
            ...
            async with self.session.get(f"{self.base_url}/api/get_all") as response:
                if response.status == 200:
                    ...
                    for item in data[:limit]:
                        results.append(MusicGenerationResult(
                            id=item.get("id", ""),
                            status=item.get("status", ""),
                            audio_url=item.get("audio_url"),
                            video_url=item.get("video_url"),
