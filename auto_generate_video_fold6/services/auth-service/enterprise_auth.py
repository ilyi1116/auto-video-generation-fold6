...
class LDAPAuthenticator:
    ...
    async def _map_user_roles(self, groups: List[str], attributes: Dict[str, Any]) -> List[UserRole]:
        """映射用戶角色"""
    ...
    async def process_saml_response(self, saml_response: str) -> Optional[User]:
        ...
        try:
            ...
            user = User(
                ...
                groups=user_attrs.get('groups', '').split(',') if user_attrs.get('groups') else [],
                attributes=user_attrs,
...
class JWTTokenManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.access_token_expire = timedelta(minutes=config.get('access_token_expire_minutes', 30))
        ...
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                """, (
                    ...
                    user.department, json.dumps([r.value for r in user.roles]),
                    json.dumps(user.groups), json.dumps(user.attributes),
                    user.provider.value, user.is_active, user.last_login,
    ...
    async def validate_session(self, session_id: str) -> Optional[AuthSession]:
        ...
        try:
            ...
            if session_data:
                ...
                self.redis_client.setex(session_key, timedelta(hours=24), 
