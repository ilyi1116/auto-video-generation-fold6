...
class VoiceCloner:
    ...
    def _calculate_quality_score(self, embeddings: List[np.ndarray], audio_samples: List[np.ndarray]) -> float:
        """計算語音品質分數"""
        scores = []
    ...
    async def _apply_voice_conversion(self, audio_data: bytes, target_embedding: np.ndarray) -> bytes:
        """應用語音轉換（簡化實現）"""
