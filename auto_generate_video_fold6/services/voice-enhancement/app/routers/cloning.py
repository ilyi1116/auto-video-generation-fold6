...
async def clone_voice_synthesis(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=cloned_voice_{request.target_voice}.wav"
            }
        )
    ...
    try:
        ...
        for i, audio_file in enumerate([audio_file1, audio_file2], 1):
            if not audio_file.content_type.startswith('audio/'):
                raise HTTPException(
                    status_code=400, 
                    detail=f"文件 {i} 不是有效的音訊格式"
                )
        
        logger.info("收到語音相似度分析請求", 
                   user_id=current_user.get("id"),
...
@router.delete("/profiles/{voice_name}",
              summary="刪除語音檔案",
