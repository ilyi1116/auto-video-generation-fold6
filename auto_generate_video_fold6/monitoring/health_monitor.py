...
class HealthMonitor:
        ...
        try:
            if service.target == "check_disk_space":
                ...
                if process.returncode == 0:
                    ...
                    status = HealthStatus.CRITICAL
                    message = stderr.decode().strip() or f"命令執行失敗 (退出代碼: {process.returncode})"
    ...
    async def check_service(self, service: ServiceCheck) -> HealthResult:
        ...
        # 自動重啟機制
        if (result.status in [HealthStatus.CRITICAL, HealthStatus.DOWN] and 
            service.auto_restart and service.restart_command):
    ...
    async def save_results(self):
        ...
        try:
            ...
            # 保存歷史記錄
            history_file = self.monitor_dir / f"health_history_{datetime.now().strftime('%Y%m%d')}.jsonl"
