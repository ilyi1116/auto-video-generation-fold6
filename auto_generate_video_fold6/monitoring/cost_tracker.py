...
class CostTracker:
    ...
    async def track_api_call(self, 
        ...
        # 計算成本
        cost = self._calculate_cost(provider, model, operation_type, 
                                  tokens_used, characters_used, images_generated)
    ...
    def _calculate_cost(self, provider: str, model: str, operation_type: str,
                       tokens_used: int, characters_used: int, images_generated: int) -> float:
        """計算 API 呼叫成本"""
    ...
    async def _update_daily_cache(self):
        ...
        if (self._last_cache_update and 
            self._last_cache_update.date() == today and
            (datetime.now() - self._last_cache_update).seconds < 300):  # 5分鐘快取
            return
    ...
    async def _check_budget_alerts(self, new_cost: float):
        ...
        if usage_rate >= 0.9:
            logger.warning(f"⚠️  預算警告: 已使用 {usage_rate:.1%} ({current_cost:.2f}/${daily_budget})")
            await self._send_budget_alert("critical", usage_rate, current_cost, daily_budget)
        elif usage_rate >= 0.8:
            logger.warning(f"📊 預算提醒: 已使用 {usage_rate:.1%} ({current_cost:.2f}/${daily_budget})")
            await self._send_budget_alert("warning", usage_rate, current_cost, daily_budget)

    async def _send_budget_alert(self, level: str, usage_rate: float, 
                               current_cost: float, budget: float):
        """發送預算警告"""
        alert_data = {
    ...
    async def check_budget_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "can_continue": not summary.is_over_budget or not self._should_stop_on_budget_exceeded(),
    ...
    async def export_cost_data(self, days: int = 30) -> str:
        ...
        # 儲存匯出資料
        export_file = Path(f"monitoring/cost_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
async def main():
    ...
    await tracker.track_api_call("stability_ai", "stable-diffusion-xl", "image_generation", images_generated=2)
    await tracker.track_api_call("elevenlabs", "voice_synthesis", "voice_synthesis", characters_used=500)
