...
class TestRunner:
    ...
    def run_python_tests(self, test_type: str = "all", coverage: bool = True, verbose: bool = True) -> Dict[str, Any]:
        """執行 Python 測試"""
...
def main():
    ...
    parser.add_argument("--type", choices=["all", "python", "frontend", "lint", "security", "health"], 
                       default="all", help="測試類型")
    parser.add_argument("--python-test-type", choices=["all", "unit", "integration", "slow", "fast"],
                       default="all", help="Python 測試類型")
    parser.add_argument("--frontend-test-type", choices=["all", "unit", "component", "e2e", "coverage"],
                       default="all", help="前端測試類型")
    parser.add_argument("--no-coverage", action="store_true", help="跳過覆蓋率收集")
    parser.add_argument("--report-format", choices=["json", "html"], default="json", help="報告格式")
