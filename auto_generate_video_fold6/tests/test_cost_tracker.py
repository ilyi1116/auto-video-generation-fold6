...
try:
    from monitoring.cost_tracker import CostTracker, get_cost_tracker, APICallRecord, DailyCostSummary
...
class TestCostTracker:
    ...
    async def test_daily_summary(self, cost_tracker):
        ...
        await cost_tracker.track_api_call("stability_ai", "stable-diffusion-xl", "image_generation", images_generated=2)
    ...
    async def test_export_cost_data(self, cost_tracker):
        ...
        # 添加測試數據
        await cost_tracker.track_api_call("openai", "gpt-4", "text_generation", tokens_used=500)
        await cost_tracker.track_api_call("stability_ai", "stable-diffusion-xl", "image_generation", images_generated=1)
...
class TestBudgetController:
    ...
    async def test_dynamic_budget_adjustment(self, budget_controller):
        ...
        if budget_controller.config_manager:
            ...
            if result:  # 如果支援動態調整
                new_budget = budget_controller.config_manager.get("cost_control.daily_budget_usd", 10.0)
