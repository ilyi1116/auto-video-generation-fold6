...
async def demo_ai_orchestrator():
    ...
    try:
        ...
        for provider, stats in status.items():
            ...
            print(f"  {health_status} {provider}: 成功率 {stats['success_rate']:.2%}, 平均響應時間 {stats['average_response_time']:.2f}s")
...
async def demo_complete_workflow():
    ...
    try:
        ...
        if suno_key:
            ...
            print(f"✅ 音樂: {music.title if music and music.status == 'completed' else '生成中或失敗'}")
        else:
