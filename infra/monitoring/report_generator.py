...
"""
    ...
    async def _generate_charts(
        ...
        charts_dir = (
            self.reports_dir
            / "charts"
            / f"{config.report_type}_{datetime.now().strftime('%Y%m%d')}"
        )
    ...
    async def _create_cost_distribution_chart(
        self, cost_data: Dict[str, Any], output_dir: Path
    ) -> Dict[str, str]:
        """創建成本分布圖"""
    ...
    async def _create_cost_trend_chart(
        self, daily_data: List[Dict[str, Any]], output_dir: Path
    ) -> Dict[str, str]:
        """創建成本趨勢圖"""
        if not daily_data:
    ...
    async def _prepare_template_data(
        ...
        return {
            ...
            "generation_charts": [
                chart
                for chart in charts
                if "generation" in chart.get("title", "").lower()
                or "platform" in chart.get("title", "").lower()
            ],
