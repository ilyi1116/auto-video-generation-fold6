    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            async def send_wrapper(message):
                if message["type"] == "http.response.start" and self.config.security_headers_enabled:
...
class SecurityManager:
    ...
    def _setup_default_permissions(self):
        """設置默認權限"""
        # 管理員權限
        self.permission_checker.add_permission("admin", "users", ["create", "read", "update", "delete"])
        self.permission_checker.add_permission("admin", "services", ["create", "read", "update", "delete"])
        self.permission_checker.add_permission("admin", "system", ["create", "read", "update", "delete"])
        
        # 用戶權限
        self.permission_checker.add_permission("user", "profile", ["read", "update"])
        self.permission_checker.add_permission("user", "videos", ["create", "read", "update", "delete"])
    ...
    def get_current_user_dependency(self):
        ...
        async def get_current_user(payload: Dict[str, Any] = Depends(security_bearer)) -> Dict[str, Any]:
            return payload
