...
class ServiceRegistry:
    ...
    async def register(self, instance: ServiceInstance) -> bool:
        ...
        async with self._lock:
            ...
            if existing:
                ...
                logger.info(f"Updated service instance: {instance.service_name}@{instance.host}:{instance.port}")
                return True
            else:
                # 添加新實例
                self._services[instance.service_name].append(instance)
                logger.info(f"Registered service instance: {instance.service_name}@{instance.host}:{instance.port}")
                return True
    ...
    async def get_healthy_instances(self, service_name: str) -> List[ServiceInstance]:
        ...
        async with self._lock:
            ...
            return [instance for instance in self._services[service_name] 
...
class HealthChecker:
    ...
    async def _check_instance_health(self, instance: ServiceInstance):
        ...
        try:
            ...
            logger.warning(f"Health check failed for {instance.service_name}@{instance.host}:{instance.port}: {e}")
            instance.status = ServiceStatus.UNHEALTHY
...
class LoadBalancer:
    ...
    async def select_instance(self, instances: List[ServiceInstance]) -> Optional[ServiceInstance]:
        ...
        healthy_instances = [instance for instance in instances 
                           if instance.status == ServiceStatus.HEALTHY]
...
class ServiceDiscovery:
    ...
    async def register_service(self, service_name: str, host: str, port: int, 
                             weight: int = 1, metadata: Dict[str, Any] = None) -> bool:
        """註冊服務"""
