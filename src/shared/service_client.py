...
from .service_discovery import ServiceInstance, get_service_discovery
...
class ServiceClient:
    ...
    async def request(
        ...
        return await self.circuit_breaker.call(
            self._request_with_retry, method, path, headers, json_data, data, params
        )
            ...
            try:
                ...
                if not instance:
                    raise ServiceUnavailableError(
                        f"No available instances for service: {self.service_name}"
                    )
                ...
                if attempt < self.retry_config.max_attempts - 1:
                    ...
                    delay = min(
                        delay * self.retry_config.exponential_base, self.retry_config.max_delay
                    )

        raise last_exception or ServiceUnavailableError(
            f"All retry attempts failed for service: {self.service_name}"
        )
