...
test.describe("專案創建功能", () => {
  ...
  test("應該能完成完整的專案創建流程", async ({ page }) => {
    ...
    // 步驟 1: 專案設定
    await expect(page.locator('[data-testid="step-indicator"]')).toContainText(
      "1 / 5",
    );
    ...
    await page.route("/api/ai/script/generate", (route) => {
      route.fulfill({
        ...
        body: JSON.stringify({
          script:
            "歡迎來到 AI 技術趨勢的介紹。今天我們將探討最新的人工智慧發展...",
          scenes: [
    ...
    await expect(
      page.locator('[data-testid="generated-images"]'),
    ).toBeVisible();
    await page.click('[data-testid="next-button"]');
  ...
  test("應該能保存草稿並稍後繼續", async ({ page }) => {
    ...
    // 驗證草稿保存成功
    await expect(
      page.locator('[data-testid="draft-saved-message"]'),
    ).toBeVisible();
    ...
    // 驗證草稿顯示在儀表板
    await expect(
      page.locator('[data-testid="draft-projects"] >> text=草稿專案'),
    ).toBeVisible();
  ...
  test("應該在載入時顯示進度指示器", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="generate-script-button"]'),
    ).toBeDisabled();
