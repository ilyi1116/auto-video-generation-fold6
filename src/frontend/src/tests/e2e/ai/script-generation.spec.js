...
test.describe("AI 腳本生成功能", () => {
  ...
  test("應該能生成基本腳本", async ({ page }) => {
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          script: {
            title: "科技趨勢分析",
            content:
              "歡迎來到今天的科技趨勢分析。在這個快速發展的數位時代，我們看到了許多令人興奮的技術突破...",
            scenes: [
              {
                id: 1,
                text: "歡迎來到今天的科技趨勢分析",
    ...
    // 驗證腳本生成結果
    await expect(
      page.locator('[data-testid="generated-script"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="script-title"]')).toContainText(
      "科技趨勢分析",
    );
    await expect(page.locator('[data-testid="script-content"]')).toContainText(
      "歡迎來到今天的科技趨勢分析",
    );
    ...
    // 驗證元數據顯示
    await expect(page.locator('[data-testid="script-duration"]')).toContainText(
      "14",
    );
    await expect(page.locator('[data-testid="word-count"]')).toContainText(
      "156",
    );
    await expect(page.locator('[data-testid="reading-time"]')).toContainText(
      "1分14秒",
    );
  });
  ...
  test("應該能編輯生成的腳本", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="scene-1"]')).toContainText(
      "編輯後的場景內容",
    );
  ...
  test("應該能添加和刪除場景", async ({ page }) => {
    ...
    await page.route("/api/ai/script/generate", (route) => {
      route.fulfill({
        ...
        body: JSON.stringify({
          script: {
            title: "場景測試腳本",
            scenes: [{ id: 1, text: "第一個場景", duration: 5, type: "intro" }],
          },
        }),
      });
    });
    ...
    // 驗證新場景已添加
    await expect(page.locator('[data-testid="scene-2"]')).toBeVisible();
    await expect(page.locator('[data-testid="scene-2"]')).toContainText(
      "新添加的場景",
    );
  ...
  test("應該能使用趨勢主題建議", async ({ page }) => {
    ...
    // 驗證趨勢主題顯示
    await expect(
      page.locator('[data-testid="trending-topics-modal"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="topic-1"]')).toContainText(
      "ChatGPT 最新功能",
    );
    await expect(page.locator('[data-testid="topic-2"]')).toContainText(
      "2024 網路安全趨勢",
    );
    await expect(page.locator('[data-testid="topic-3"]')).toContainText(
      "居家辦公生產力技巧",
    );
  ...
  test("應該能保存腳本到專案", async ({ page }) => {
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          script: {
            title: "保存測試腳本",
            content: "這是要保存的腳本內容",
            scenes: [{ id: 1, text: "測試場景", duration: 5, type: "content" }],
          },
        }),
      });
    });
    ...
    await expect(
      page.locator('[data-testid="save-success-message"]'),
    ).toContainText("腳本已保存到專案");

    // 驗證前往專案的連結
    await expect(
      page.locator('[data-testid="view-project-link"]'),
    ).toBeVisible();
  });
  ...
  test("應該支援腳本模板", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="template-educational"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="template-entertainment"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="template-promotional"]'),
    ).toBeVisible();
    ...
    // 驗證模板已套用到表單
    await expect(page.locator('[data-testid="tone-select"]')).toHaveValue(
      "educational",
    );
    await expect(page.locator('[data-testid="platform-select"]')).toHaveValue(
      "youtube",
    );

    // 驗證模板提示文字
    await expect(page.locator('[data-testid="template-hint"]')).toContainText(
      "教育內容建議",
    );
  });
