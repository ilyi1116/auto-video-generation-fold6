...
test.describe('用戶完整流程', () => {
  ...
  test('完整影片創建流程', async ({ page }) => {
    ...
    await page.fill('[data-testid="project-description"]', '這是一個測試影片的描述');
    ...
    await expect(page.locator('[data-testid="script-content"]')).not.toBeEmpty();
    ...
    // 等待圖像生成完成
    await expect(page.locator('[data-testid="generated-images"]')).toBeVisible({ timeout: 60000 });
    await expect(page.locator('[data-testid="image-item"]')).toHaveCount(3);
    
    // 選擇第一張圖片
    await page.click('[data-testid="image-item"]:first-child [data-testid="select-image"]');
    ...
    // 等待語音合成完成
    await expect(page.locator('[data-testid="audio-player"]')).toBeVisible({ timeout: 45000 });
    ...
    await expect(page.locator('[data-testid="video-preview"]')).toBeVisible({ timeout: 120000 });
    ...
    await expect(page.locator('[data-testid="project-item"]').first()).toContainText('我的測試影片');
  });
  ...
  test('AI 腳本生成功能', async ({ page }) => {
    ...
    // 檢查腳本內容
    const scriptContent = await page.locator('[data-testid="script-content"]').textContent();
    expect(scriptContent.length).toBeGreaterThan(50);
    
    // 檢查腳本統計
    await expect(page.locator('[data-testid="word-count"]')).toContainText(/\d+ 字/);
    await expect(page.locator('[data-testid="estimated-duration"]')).toContainText(/\d+ 秒/);
    
    // 測試腳本編輯
    await page.click('[data-testid="edit-script-button"]');
    await page.fill('[data-testid="script-editor"]', scriptContent + '\n\n這是額外添加的內容。');
    ...
    await expect(page.locator('[data-testid="save-success-message"]')).toContainText('腳本已保存到資料庫');
    
    // 檢查腳本歷史
    await page.click('[data-testid="script-history-tab"]');
    await expect(page.locator('[data-testid="history-item"]').first()).toContainText('健康飲食腳本');
  });
  ...
  test('圖像生成和管理', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="generated-images"]')).toBeVisible({ timeout: 90000 });
    ...
    await expect(firstImage.locator('[data-testid="favorite-icon"]')).toHaveClass(/active/);
    ...
    await expect(page.locator('[data-testid="variant-images"]')).toBeVisible({ timeout: 60000 });
    
    // 檢查圖像庫
    await page.click('[data-testid="image-library-tab"]');
    await expect(page.locator('[data-testid="library-image"]')).toHaveCountGreaterThan(0);
  ...
  test('社群媒體發布流程', async ({ page }) => {
    ...
    // 檢查平台連接狀態
    await expect(page.locator('[data-testid="platform-connections"]')).toBeVisible();
    ...
    await page.fill('[data-testid="post-description"]', '這是一個關於人工智慧的教育影片，希望大家喜歡！');
    ...
    await expect(page.locator('[data-testid="publish-success"]')).toContainText('貼文已排程發布');
    
    // 檢查排程列表
    await page.click('[data-testid="scheduled-posts-tab"]');
    await expect(page.locator('[data-testid="scheduled-item"]').first()).toContainText('我的精彩影片分享');
  ...
  test('用戶設定和個人檔案管理', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="save-success"]')).toContainText('個人資訊已更新');
    ...
    await expect(page.locator('[data-testid="password-changed"]')).toContainText('密碼已成功更新');
    
    // 檢查個人檔案頁面
    await page.goto('/profile');
    await expect(page.locator('[data-testid="user-display-name"]')).toContainText('測試用戶');
    await expect(page.locator('[data-testid="user-bio"]')).toContainText('我是一個熱愛創作的內容創作者');
  ...
  test('響應式設計測試', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="main-content"]')).toHaveCSS('margin-left', /\d+px/);
