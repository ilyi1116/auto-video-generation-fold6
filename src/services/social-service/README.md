...
```bash
# 1. 安裝依賴
cd src/services/social-service
pip install -e .
...
# 運行容器
docker run -p 8006:8006 --env-file .env social-service
```
...
預期響應：
```json
{"status": "healthy", "service": "social-service", "version": "1.0.0"}
```
