...
```bash
# 1. 安裝依賴
cd src/services/voice-enhancement
pip install -e .
...
```bash
# 構建鏡像
docker build -t voice-enhancement .

# 運行容器
docker run -p 8013:8013 --env-file .env voice-enhancement
```
...
```bash
# 本地開發
tail -f logs/voice-enhancement.log

# Docker 環境
docker logs -f voice-enhancement

# Kubernetes 環境
kubectl logs -f deployment/voice-enhancement
```
