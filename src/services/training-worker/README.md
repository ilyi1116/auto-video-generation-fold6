...
```bash
# 1. 安裝依賴
cd src/services/training-worker
pip install -e .
...
```bash
# 構建鏡像
docker build -t training-worker .

# 運行容器
docker run -p 8010:8010 --env-file .env training-worker
```
...
預期響應：
```json
{"status": "healthy", "service": "training-worker", "version": "1.0.0"}
```
...
```bash
# 本地開發
tail -f logs/training-worker.log

# Docker 環境
docker logs -f training-worker
