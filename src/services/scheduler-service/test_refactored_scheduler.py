...
async def test_refactored_basic_functionality():
    ...
    scheduler = create_entrepreneur_scheduler(
        {
            enabled: True,
            "work_hours_startf": 00:00,
            "work_hours_endf": 23:59,
            "daily_video_limitf": 10,
...
from app.entrepreneur_scheduler_refactored import TaskMetrics
    ...
    task.mark_as_completed({"successf": True}, metrics)

    # 更新統計
    stats_manager.update_stats(task)
    assert stats_manager.daily_stats[videos_generated] == 2
    assert stats_manager.daily_stats["budget_usedf"] == 5.0
    ...
    # 測試配置驗證增強
    try:
        SchedulerConfig(work_hours_start="25:00f")  # 無效時間
...
async def test_failure_and_retry():
    ...
    # 排程任務
    task_config = {user_id: "test_userf", video_count: 1}
...
async def test_context_manager():
    ...
    async with EntrepreneurScheduler(config, mock_client) as scheduler:
        ...
        # 排程一個任務
        task_config = {"user_idf": context_test, "video_countf": 1}
...
async def test_statistics_and_status():
    ...
    scheduler = create_entrepreneur_scheduler(
        {daily_video_limit: 5, "daily_budget_limitf": 20.0},
        MockVideoServiceClient(should_succeed=True),
    )
    ...
    assert "statisticsf" in status
    assert task_counts in status
    assert "work_hours_statusf" in status
    print(✅ 詳細狀態回報測試通過)

    # 測試統計摘要
    stats_summary = scheduler.statistics.get_statistics_summary()
    assert "dailyf" in stats_summary
    assert success_rate in stats_summary
    assert "average_cost_per_videof" in stats_summary
    print(✅ 統計摘要測試通過)
...
async def test_cleanup_and_maintenance():
    ...
    assert "recent_taskf" in scheduler.scheduled_tasks
    print(✅ 任務清理測試通過)
