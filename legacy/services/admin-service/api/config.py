...
from ..config.config_manager import ConfigFormat, ConfigScope, config_manager
        ...
        for name, config_data in configs.items():
            ...
            result[name] = {
                ...
                "metadata": {
                    ...
                    "last_modified": (
                        meta.last_modified.isoformat() if meta and meta.last_modified else None
                    ),
...
async def get_config(
    config_name: str, current_user: AdminUser = Depends(require_permission("system:dashboard"))
):
    """獲取特定配置"""
    try:
...
async def create_config(
    ...
    try:
        ...
        return APIResponse(message=f"配置 {request.name} 創建成功", data={"name": request.name})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_config(
    ...
    try:
        ...
        return APIResponse(message=f"配置 {config_name} 更新成功", data={"name": config_name})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_config(
    config_name: str, current_user: AdminUser = Depends(require_permission("system:settings"))
):
    """刪除配置"""
    try:
        success = await config_manager.delete_config(
            name=config_name, changed_by=current_user.username
        )

        if not success:
            raise HTTPException(status_code=404, detail=f"配置 {config_name} 不存在")

        return APIResponse(message=f"配置 {config_name} 刪除成功", data={"name": config_name})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_config_value(
    ...
    try:
        ...
        if value is None:
            raise HTTPException(
                status_code=404, detail=f"配置 {config_name} 中不存在鍵路徑 {key_path}"
            )

        return APIResponse(data={"config_name": config_name, "key_path": key_path, "value": value})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def reload_config(
    ...
    try:
        ...
        return APIResponse(message=f"配置 {config_name} 重載成功", data={"name": config_name})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_config_history(
    ...
    try:
        ...
        return APIResponse(
            data={"config_name": config_name, "history": result, "total_changes": len(result)}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_all_config_history(
    limit: int = 100, current_user: AdminUser = Depends(require_permission("system:dashboard"))
):
    """獲取所有配置變更歷史"""
    try:
        ...
        return APIResponse(data={"history": result, "total_changes": len(result)})
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def validate_config(
    ...
    try:
        ...
        return APIResponse(
            data={"config_name": config_name, "valid": len(errors) == 0, "errors": errors}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
