...
from ..logs import (
    ...
    init_log_analyzer, init_anomaly_detector,
    init_log_predictor, init_log_aggregator,
    init_log_alerting
)
...
def generate_predictions_task(self, prediction_params: Dict[str, Any]):
    ...
    try:
        ...
        predictions = asyncio.run(
            log_predictor.generate_predictions(
...
def generate_dashboard_task(self, dashboard_params: Dict[str, Any]):
    ...
    try:
        ...
        dashboard_data = asyncio.run(
            log_aggregator.create_dashboard_data(
...
def generate_report_task(self, report_params: Dict[str, Any]):
    ...
    try:
        ...
        # 生成報告
        report = asyncio.run(
            log_aggregator.generate_report(
...
def automated_analysis_task(self):
    ...
    try:
        ...
        try:
            anomalies = asyncio.run(
                anomaly_detector.detect_anomalies(hours=1)
            )
            results["anomalies"] = {
                "count": len(anomalies),
                "high_severity_count": len([a for a in anomalies if a.severity.value in ["high", "critical"]])
            }
        except Exception as e:
