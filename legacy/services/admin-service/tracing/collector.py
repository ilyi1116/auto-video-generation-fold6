...
class TraceCollector:
    ...
    def __init__(self, storage_path: str = "/data/data/com.termux/files/home/myProject/traces", 
                 max_memory_traces: int = 10000, batch_size: int = 100):
    ...
    async def get_traces(self, trace_id: Optional[str] = None, 
        ...
        for trace in reversed(self.memory_traces):
            ...
            if self._match_filters(trace, trace_id, service_name, 
                                 operation_category, start_time, end_time):
                traces.append(trace)
        
        return traces
    
    def _match_filters(self, trace: Dict[str, Any], trace_id: Optional[str],
                      service_name: Optional[str], operation_category: Optional[str],
    ...
    async def export_traces(self, output_path: str, 
        ...
        try:
            ...
            if start_date and end_date:
                ...
                while current_dt <= end_dt:
                    file_path = self.storage_path / f"traces_{current_dt.strftime('%Y-%m-%d')}.jsonl"
                    if file_path.exists():
