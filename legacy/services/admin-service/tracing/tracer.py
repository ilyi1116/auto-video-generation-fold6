...
class CustomTracer:
    ...
    def finish_span(
        self, context: TraceContext, status: str = "success", error: Optional[Exception] = None
    ):
        """結束 Span"""
        if context.span_id in self.spans_cache:
            ...
            logger.debug(f"結束 Span: {span_info['name']} " f"(耗時: {span_info['duration']:.3f}s)")
...
def trace_request(
    operation_name: str = None, include_headers: bool = False, include_params: bool = False
):
    """HTTP 請求追蹤裝飾器"""
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if request:
                ...
                context.add_attribute("http.user_agent", request.headers.get("user-agent", ""))

                if include_headers:
                    context.add_attribute("http.headers", dict(request.headers))

                if include_params and request.query_params:
                    context.add_attribute("http.query_params", dict(request.query_params))
...
def trace_function(
    operation_name: str = None, capture_args: bool = False, capture_result: bool = False
):
    """通用函數追蹤裝飾器"""
...
def finish_span(context: TraceContext, status: str = "success", error: Optional[Exception] = None):
    """手動結束 Span"""
