...
from .crud import crud_system_log, crud_trending_keyword
...
from .schemas import SystemLogCreate, TrendingKeywordCreate
...
class SocialTrendsManager:
    ...
    async def collect_all_trends(self) -> Dict[str, Any]:
        ...
        try:
            # 獲取所有活躍的趨勢配置
            active_configs = (
                db.query(SocialTrendConfig).filter(SocialTrendConfig.is_active == True).all()
            )
    ...
    async def _save_trends_to_db(
        self, trends: List[Dict[str, Any]], config: SocialTrendConfig, timeframe: TrendTimeframe
    ) -> int:
        """保存趨勢數據到資料庫"""
        db = SessionLocal()
        saved_count = 0
    ...
    async def collect_platform_trends(
        self, platform: SocialPlatform, region: str = "global"
    ) -> Dict[str, Any]:
        """收集指定平台和地區的趨勢"""
        db = SessionLocal()
        ...
        try:
            ...
            if not config:
                return {"success": False, "error": f"找不到 {platform} {region} 的活躍配置"}
...
async def collect_platform_trends_manually(
    platform: SocialPlatform, region: str = "global"
) -> Dict[str, Any]:
    """手動收集指定平台趨勢"""
