        ...
        try:
            ...
            if level in [LogLevel.ERROR, LogLevel.CRITICAL]:
                ...
                if (
                    ...
                    or (datetime.utcnow() - self._last_batch_time).seconds >= self.batch_timeout
                ):
...
class PerformanceLogger:
    ...
    async def log_performance_metric(
        self, metric_name: str, value: float, unit: str = "ms", details: Dict[str, Any] = None
    ):
        """記錄性能指標"""
        ...
        await structured_logger.log(
            ...
            details={"metric_name": metric_name, "value": value, "unit": unit, **(details or {})},
            level=LogLevel.INFO,
        )

    async def log_slow_query(self, query: str, duration_ms: int, table_name: str = None):
        """記錄慢查詢"""
    ...
    def time_operation(self, operation_name: str):
        ...
        try:
            ...
            # 異步記錄性能日誌
            asyncio.create_task(self.log_performance_metric(operation_name, duration_ms))
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                await AuditLogger.log_error(error=e, context=f"函數調用: {function_name}")
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                asyncio.create_task(
                    AuditLogger.log_error(error=e, context=f"函數調用: {function_name}")
                )
...
class LogAnalyzer:
    ...
    async def get_error_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            # 按錯誤類型分組
            error_types = (
                db.query(SystemLog.error_code, db.func.count(SystemLog.id).label("count"))
                .filter(
                    SystemLog.level.in_([LogLevel.ERROR, LogLevel.CRITICAL]),
                    SystemLog.created_at >= start_time,
                    SystemLog.error_code.isnot(None),
                )
                .group_by(SystemLog.error_code)
                .all()
            )

            # 按資源類型分組
            resource_errors = (
                db.query(SystemLog.resource_type, db.func.count(SystemLog.id).label("count"))
                .filter(
                    SystemLog.level.in_([LogLevel.ERROR, LogLevel.CRITICAL]),
                    SystemLog.created_at >= start_time,
                )
                .group_by(SystemLog.resource_type)
                .all()
            )
            ...
            return {
                ...
                "resource_errors": [
                    {"resource": re.resource_type, "count": re.count} for re in resource_errors
                ],
            }
    ...
    async def get_performance_metrics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            # 平均響應時間
            avg_response_time = (
                db.query(db.func.avg(SystemLog.duration_ms))
                .filter(
            ...
            request_count = (
                db.query(SystemLog)
                .filter(SystemLog.action == "api_request", SystemLog.created_at >= start_time)
                .count()
            )
...
async def export_logs(
    start_date: datetime, end_date: datetime, level: LogLevel = None, format: str = "json"
) -> str:
    """導出日誌"""
    db = SessionLocal()
    ...
    try:
        query = db.query(SystemLog).filter(
            SystemLog.created_at >= start_date, SystemLog.created_at <= end_date
        )
