...
from .models import (
    ...
    UserSegment, DeviceType, BehaviorPattern, BehaviorInsight
)
...
class BehaviorAnalyzer:
    ...
    def _calculate_user_metrics(self, 
        ...
        return {
            ...
            "conversion_metrics": {
                ...
                "conversion_value": sum(a.metadata.get("conversion_value", 0) for a in conversion_actions)
            },
    ...
    def _generate_user_recommendations(self, metrics: Dict[str, Any], profile: UserProfile) -> List[str]:
        """生成用戶建議"""
        recommendations = []
    ...
    async def analyze_cohort_behavior(self, 
                                    cohort_name: str,
                                    days: int = 30) -> AnalysisResult:
        """分析用戶群組行為"""
    ...
    async def analyze_funnel_performance(self,
        ...
        for i in range(len(conversions) - 1):
            ...
            drop_off_rate = 1 - (next_step["users"] / current_step["users"]) if current_step["users"] > 0 else 0
            ...
            drop_off_analysis.append({
                ...
                "severity": "high" if drop_off_rate > 0.5 else "medium" if drop_off_rate > 0.3 else "low"
            })
        ...
        # 找出最大流失點
        max_drop_off = max(drop_off_analysis, key=lambda x: x["drop_off_rate"]) if drop_off_analysis else None
        if max_drop_off:
            insights.append(f"最大流失點在 {max_drop_off['from_step']} 到 {max_drop_off['to_step']}")
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "avg_drop_off_rate": statistics.mean([d["drop_off_rate"] for d in drop_off_analysis]) if drop_off_analysis else 0
            },
            "insights": insights,
            "recommendations": recommendations,
    ...
    async def analyze_page_performance(self, 
        ...
        for url, stats in page_stats.items():
            ...
            analysis = {
                ...
                "avg_views_per_user": round(stats["total_views"] / unique_users_count, 2) if unique_users_count > 0 else 0,
                "bounce_rate": round(stats["bounce_count"] / stats["total_views"], 2) if stats["total_views"] > 0 else 0,
                "avg_time_on_page_seconds": round(stats["total_time_spent"] / stats["total_views"] / 1000, 2) if stats["total_views"] > 0 else 0
            }
            
            if stats["load_times"]:
                analysis["avg_load_time_ms"] = round(statistics.mean(stats["load_times"]), 2)
                analysis["p95_load_time_ms"] = np.percentile(stats["load_times"], 95) if len(stats["load_times"]) > 1 else stats["load_times"][0]
            
            page_analysis[url] = analysis
        
        # 排序並生成洞察
        sorted_pages = sorted(
            page_analysis.values(),
        ...
        if sorted_pages:
            ...
            insights.append(f"最受歡迎的頁面是 {top_page['url']}，共 {top_page['total_views']} 次瀏覽")
        ...
        return {
            ...
            "summary": {
                ...
                "avg_bounce_rate": round(statistics.mean([p["bounce_rate"] for p in page_analysis.values()]), 2) if page_analysis else 0
            },
            "insights": insights,
            "recommendations": recommendations,
            "analysis_timestamp": datetime.utcnow().isoformat()
    ...
    async def detect_anomalies(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        historical_actions = [
            a for a in self.collector.actions
            if historical_start <= a.timestamp <= historical_end
        ]
        ...
        # 檢測錯誤率異常
        current_errors = len([a for a in current_period_actions if a.action_type == ActionType.ERROR])
        historical_errors = len([a for a in historical_actions if a.action_type == ActionType.ERROR])
        
        current_error_rate = current_errors / max(current_traffic, 1)
        historical_error_rate = historical_errors / max(historical_traffic, 1)
        
        if current_error_rate > historical_error_rate * 2 and current_error_rate > 0.05:  # 錯誤率翻倍且超過5%
            anomalies.append({
