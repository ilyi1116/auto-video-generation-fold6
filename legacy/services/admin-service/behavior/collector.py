...
class BehaviorCollector:
    ...
    async def _get_or_create_session(
        self, user_id: str, request_info: Optional[Dict[str, Any]] = None
    ) -> BehaviorSession:
        """獲取或創建用戶會話"""
        now = datetime.utcnow()
    ...
    async def _update_user_profile(
        self, user_id: str, session: BehaviorSession, action: UserAction
    ):
        """更新用戶檔案"""
    ...
    def _calculate_user_segment(self, profile: UserProfile) -> UserSegment:
        ...
        # 高級用戶（高參與度和使用頻率）
        if (
            profile.engagement_score > 0.7
            and profile.total_sessions > 20
            and profile.avg_actions_per_session > 10
        ):
    ...
    async def get_page_views(
        self, hours: int = 24, page_url: Optional[str] = None
    ) -> List[UserAction]:
        """獲取頁面瀏覽數據"""
    ...
    async def get_conversion_funnel(
        ...
        for i, step in enumerate(funnel_steps[1:], 1):
            ...
            for action in reversed(self.actions):
                ...
                if action.user_id in prev_users and self._action_matches_condition(action, step):
    ...
    async def _cleanup_old_data(self):
        ...
        # 清理舊會話
        old_sessions = [
            sid for sid, session in self.sessions.items() if session.start_time < cutoff_time
        ]
