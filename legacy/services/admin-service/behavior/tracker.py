...
class BehaviorTracker:
    ...
    async def track_search(
        ...
        """追蹤搜索事件"""
        metadata = {"query": query, "query_length": len(query)}
    ...
    async def track_conversion(
        ...
        conversion_metadata.update({"conversion_type": conversion_type, "is_conversion": True})
    ...
    def _update_heatmap_data(self, action: UserAction):
        ...
        if action.coordinates and action.page_url:
            ...
            # 新位置
            page_heatmap.append(
                {"x": x, "y": y, "count": 1, "action_type": action.action_type.value}
            )
    ...
    async def get_ab_test_results(self, test_id: str) -> Dict[str, Any]:
        ...
        for variant in test_config["variants"]:
            ...
            variant_stats[variant] = {
                ...
                "avg_value": (
                    statistics.mean([d["value"] for d in variant_data]) if variant_data else 0
                ),
            }
    ...
    async def get_cohort_analysis(self, cohort_name: str, days: int = 30) -> Dict[str, Any]:
        ...
        if cohort_stats["active_users"] > 0:
            cohort_stats["avg_actions_per_user"] = (
                cohort_stats["total_actions"] / cohort_stats["active_users"]
            )
    ...
    def add_alert_rule(
        self, rule_name: str, condition: Dict[str, Any], threshold: float, callback: Callable
    ):
        """添加預警規則"""
    ...
    def _evaluate_alert_condition(
        self, action: UserAction, condition: Dict[str, Any], threshold: float
    ) -> bool:
        """評估預警條件"""
        ...
        if condition_type == "error_rate":
            ...
            recent_actions = [
                a
                for a in list(self.collector.actions)[-100:]  # 最近100個動作
    ...
    async def _update_real_time_stats(self):
        ...
        page_views = sum(
            1
            for action in self.collector.actions
            if (action.action_type == ActionType.PAGE_VIEW and action.timestamp >= one_minute_ago)
        )
    ...
    async def get_real_time_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_hour": {
                ...
                "avg_page_views_per_minute": (
                    statistics.mean(current_page_views[-60:]) if current_page_views else 0
                ),
    ...
    async def get_user_journey(self, user_id: str, days: int = 7) -> Dict[str, Any]:
        ...
        journey = {
            "user_id": user_id,
            "date_range": {"start": start_date.isoformat(), "end": datetime.utcnow().isoformat()},
        ...
        for session_id, session_actions in sessions_dict.items():
            ...
            session_info = {
                ...
                "duration_minutes": (
                    session_actions[-1].timestamp - session_actions[0].timestamp
                ).seconds
                / 60,
                "actions": [
                    {
