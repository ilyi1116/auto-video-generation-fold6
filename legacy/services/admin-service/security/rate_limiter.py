...
class RateLimiter:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/3"):
        ...
        self.default_limits = {
            LimitType.IP: RateLimit(
                limit=1000,
                window_seconds=3600,
                description="每小時每個 IP 1000 次請求"
            ),
            LimitType.USER: RateLimit(
                limit=5000,
                window_seconds=3600,
                description="每小時每個用戶 5000 次請求"
            ),
            LimitType.API_KEY: RateLimit(
                limit=10000,
                window_seconds=3600,
                description="每小時每個 API Key 10000 次請求"
            ),
            LimitType.ENDPOINT: RateLimit(
                limit=100,
                window_seconds=60,
                description="每分鐘每個端點 100 次請求"
            ),
            LimitType.GLOBAL: RateLimit(
                limit=50000,
                window_seconds=3600,
                description="全局每小時 50000 次請求"
            )
        }
        ...
        self.endpoint_limits = {
            "/admin/auth/login": RateLimit(
                limit=5,
                window_seconds=300,
                description="登錄端點每5分鐘5次嘗試"
            ),
            "/admin/ai-providers": RateLimit(
                limit=200,
                window_seconds=3600,
                description="AI Provider 管理每小時200次"
            ),
            "/admin/logs/export": RateLimit(
                limit=10,
                window_seconds=3600,
                description="日誌導出每小時10次"
            ),
            "/admin/monitoring/health/check": RateLimit(
                limit=60,
                window_seconds=3600,
                description="健康檢查每小時60次"
            )
        }
    ...
    async def _check_limit_memory(self, key: str, limit: RateLimit) -> RateLimitResult:
        ...
        reset_time = datetime.fromtimestamp(
            ...
            if self._memory_store[key] else now + limit.window_seconds
        )
    ...
    async def check_rate_limit(self, request: Request) -> Optional[RateLimitResult]:
        ...
        try:
            ...
            # 1. IP 限制
            checks.append((
                self._get_key(LimitType.IP, ip),
                self.default_limits[LimitType.IP]
            ))
            
            # 2. 用戶限制
            if user_id:
                checks.append((
                    self._get_key(LimitType.USER, user_id),
                    self.default_limits[LimitType.USER]
                ))
            ...
            # 5. 全局限制
            checks.append((
                self._get_key(LimitType.GLOBAL, "all"),
                self.default_limits[LimitType.GLOBAL]
            ))
    ...
    async def _log_rate_limit_violation(self, request: Request, limit: RateLimit, result: RateLimitResult):
        """記錄限流違規"""
        try:
