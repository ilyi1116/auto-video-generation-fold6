...
class HealthMonitor:
    ...
    async def check_api_endpoints(self) -> HealthCheck:
        ...
        if failed_endpoints > 0:
            status = HealthStatus.CRITICAL if failed_endpoints == len(endpoints) else HealthStatus.WARNING
    ...
    async def check_dependencies(self) -> HealthCheck:
        ...
        for dep in dependencies:
            try:
                ...
                failed_deps += 1
                metrics.append(HealthMetric(
                    name=f"{dep['name']}_error",
                    value=str(e),
                    description=f"{dep['name']} 錯誤"
                ))
    ...
    async def process_component_alerts(self, check: HealthCheck):
        ...
        for metric in check.metrics:
            ...
            if metric_status in [HealthStatus.WARNING, HealthStatus.CRITICAL]:
                # 生成或更新告警
                level = AlertLevel.WARNING if metric_status == HealthStatus.WARNING else AlertLevel.CRITICAL
                threshold = metric.threshold_warning if metric_status == HealthStatus.WARNING else metric.threshold_critical
                ...
                # 如果是新告警或級別升級，發送通知
                if alert_key not in self.active_alerts or self.active_alerts[alert_key].level != level:
