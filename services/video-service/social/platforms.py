...
class TikTokClient(SocialPlatform):
    ...
    async def _upload_video(self, video_url: str) -> Dict[str, Any]:
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(f"TikTok upload API error: {response.status} - " f"{error_text}")
    ...
    async def _create_post(self, video_id: str, request: PublishRequest) -> Dict[str, Any]:
        ...
        async with session.post(
            ...
            if response.status != 200:
                error_text = await response.text()
                raise Exception(f"TikTok publish API error: {response.status} - " f"{error_text}")
...
class YouTubeClient(SocialPlatform):
    ...
    async def publish_video(self, request: PublishRequest) -> PublishResult:
        ...
        try:
            ...
            return PublishResult(
                ...
                url=(f"https://www.youtube.com/watch?v=" f"{result['video_id']}"),
                published_at=datetime.utcnow(),
                metadata=result,
            )
    ...
    async def _upload_video_with_metadata(self, request: PublishRequest) -> Dict[str, Any]:
        ...
        async with session.post(
            f"{self.upload_url}/videos?uploadType=multipart&" f"part=snippet,status",
            data=body,
            headers=headers,
        ) as response:
            if response.status != 200:
                error_text = await response.text()
                raise Exception(f"YouTube upload API error: {response.status} - " f"{error_text}")
...
class InstagramClient(SocialPlatform):
    ...
    async def _create_media_container(self, request: PublishRequest) -> str:
        ...
        async with session.post(
            ...
            if response.status != 200:
                error_text = await response.text()
                raise Exception(
                    f"Instagram media creation error: {response.status} - " f"{error_text}"
                )
    ...
    async def _publish_media(self, container_id: str) -> Dict[str, Any]:
        ...
        async with session.post(
            ...
            if response.status != 200:
                error_text = await response.text()
                raise Exception(f"Instagram publish error: {response.status} - " f"{error_text}")
    ...
    async def get_video_stats(self, platform_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            params = {
                "fields": (
                    "insights.metric(reach,impressions,video_views," "likes,comments,shares,saves)"
                ),
                "access_token": self.api_key,
            }
