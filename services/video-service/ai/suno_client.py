...
class SunoAIClient:
    ...
    async def _wait_for_completion(
        self, generation_id: str, max_wait: int = 300
        ...
        while True:
            try:
                async with session.get(f"{self.base_url}/generate/{generation_id}") as response:
                    if response.status != 200:
                        raise Exception(f"Failed to check generation status: " f"{response.status}")
                    ...
                    if status == "completed":
                        ...
                        raise Exception(
                            f"Voice generation failed: " f"{result.get('error', 'Unknown error')}"
                        )
                        ...
                        if elapsed > max_wait:
                            raise Exception(
                                f"Voice generation timeout after " f"{max_wait} seconds"
                            )
        ...
        try:
            ...
            async with session.post(f"{self.base_url}/clone", json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(
                        f"Voice cloning API error: {response.status} - " f"{error_text}"
                    )
