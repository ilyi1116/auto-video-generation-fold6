...
class VideoComposer:
    ...
    async def create_video(
        ...
        try:
            ...
            music_path = await self._download_media(music_url, f"{composition_id}_music.mp3") if music_url else None
    ...
    async def _create_preview(
        ...
        cmd.extend([
            ...
            "-c:v", "libx264",
            "-preset", "fast",  # Fast preset for preview
            "-crf", "28",  # Lower quality for preview
            "-c:a", "aac",
            "-b:a", "128k",
            "-r", "24",  # Lower fps for preview
            "-t", str(sum(scene.duration for scene in scenes)),
            preview_path
        ])
    ...
    async def _render_final_video(
        ...
        cmd.extend([
            ...
            "-crf", settings["crf"],
            "-c:a", "aac",
            "-b:a", platform_settings["audio_bitrate"],
            "-r", str(platform_settings["fps"]),
            "-s", platform_settings["resolution"],
            final_path
        ])
