...
@require_permission("system:dashboard")
async def get_blacklist(current_user: AdminUser = Depends(require_permission("system:dashboard"))):
    """獲取黑名單 IP"""
    try:
...
async def add_to_blacklist(
    ...
    """添加 IP 到黑名單"""
    try:
        rate_limiter.add_to_blacklist(request_data.ip_address, request_data.duration_hours or 24)
...
async def remove_from_blacklist(
    ip_address: str, current_user: AdminUser = Depends(require_permission("system:settings"))
):
    """從黑名單移除 IP"""
    try:
...
@require_permission("system:dashboard")
async def get_whitelist(current_user: AdminUser = Depends(require_permission("system:dashboard"))):
    """獲取白名單 IP"""
    try:
...
async def remove_from_whitelist(
    ...
    try:
        ...
        return APIResponse(
            message=f"已將 IP {ip_address} 從白名單移除", data={"ip_address": ip_address}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_threat_analysis(
    hours: int = 24, current_user: AdminUser = Depends(require_permission("system:dashboard"))
):
    """獲取威脅分析"""
    try:
...
async def test_rate_limit(
    request: Request, current_user: AdminUser = Depends(require_permission("system:dashboard"))
):
    """測試限流功能"""
    try:
        # 檢查當前請求的限流狀態
