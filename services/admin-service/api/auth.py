...
from ..auth.middleware import (
    ...
    require_any_permission, require_superuser
)
...
async def change_password(
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": result.get("message", "密碼更改成功")
            }
        else:
...
async def reset_password_confirm(request: ResetPasswordConfirmRequest):
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": result.get("message", "密碼重置成功")
            }
        else:
...
async def assign_role_to_user(
    ...
    """為用戶分配角色"""
    try:
        success = role_manager.assign_role_to_user(
...
async def get_auth_stats(
    current_user: Dict[str, Any] = Depends(require_permission("system:read"))
):
    """獲取認證系統統計"""
    try:
...
async def resolve_security_alert(
    alert_id: str,
    current_user: Dict[str, Any] = Depends(require_permission("security:manage"))
):
    """解決安全警報"""
    try:
