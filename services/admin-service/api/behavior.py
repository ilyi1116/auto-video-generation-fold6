...
async def analyze_funnel(
    ...
    try:
        ...
        return {"status": "success", "funnel_analysis": analysis}
...
async def analyze_page_performance(
    ...
    """分析頁面性能"""
    try:
        analysis = await behavior_analyzer.analyze_page_performance(page_url=page_url, hours=hours)

        return {"status": "success", "page_analysis": analysis}
...
async def detect_sequential_patterns(
    ...
    try:
        patterns = await pattern_detector.detect_sequential_patterns(
            min_support=min_support, max_length=max_length, time_window_hours=time_window_hours
        )
...
async def get_patterns(
    ...
    try:
        patterns = await pattern_detector.get_all_patterns(
            pattern_type=pattern_type, min_confidence=min_confidence
        )
...
async def implement_insight(
    insight_id: str, current_user: AdminUser = Depends(require_permission("behavior:manage"))
):
    """標記洞察為已實施"""
    try:
...
async def create_ab_test(
    request: ABTestRequest, current_user: AdminUser = Depends(require_permission("behavior:manage"))
):
    """創建 A/B 測試"""
    try:
...
async def assign_user_to_variant(
    ...
    try:
        ...
        return {"status": "success", "test_id": test_id, "user_id": user_id, "variant": variant}
...
async def get_ab_test_results(
    test_id: str, current_user: AdminUser = Depends(require_permission("behavior:read"))
):
    """獲取 A/B 測試結果"""
    try:
...
async def create_cohort(
    request: CohortRequest, current_user: AdminUser = Depends(require_permission("behavior:manage"))
):
    """創建用戶分群"""
    try:
        behavior_tracker.create_user_cohort(
            cohort_name=request.cohort_name, user_ids=request.user_ids
        )
...
async def get_active_users(
    ...
    try:
        ...
        return {"status": "success", "active_users": count, "time_window_minutes": minutes}
...
async def export_behavior_data(
    ...
    try:
        data = await behavior_collector.export_data(
            user_id=user_id, start_date=start_date, end_date=end_date, format=format
        )

        return {"status": "success", "data": data, "format": format}
