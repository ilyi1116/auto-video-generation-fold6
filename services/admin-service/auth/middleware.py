...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, config: Optional[Dict[str, Any]] = None):
        ...
        # 排除認證的路徑前綴
        self.excluded_prefixes = self.config.get("excluded_prefixes", ["/static", "/assets"])
...
def get_current_user(
    ...
    try:
        ...
        if not payload:
            raise HTTPException(
                status_code=401, detail="無效的認證令牌", headers={"WWW-Authenticate": "Bearer"}
            )
        ...
        raise HTTPException(
            status_code=401, detail="認證失敗", headers={"WWW-Authenticate": "Bearer"}
        )
...
def require_permission(permission: str, resource_id_param: Optional[str] = None):
    ...
    def permission_checker(
        ...
        if not permission_manager.check_permission(user_id, permission, resource_id):
            raise HTTPException(status_code=403, detail=f"用戶沒有權限: {permission}")
...
def require_any_permission(*permissions: str):
    ...
    def permission_checker(current_user: Dict[str, Any] = Depends(get_current_user)):
        ...
        if not permission_manager.check_any_permission(user_id, list(permissions)):
            raise HTTPException(
                status_code=403, detail=f"用戶沒有以下任一權限: {', '.join(permissions)}"
            )
...
def require_all_permissions(*permissions: str):
    ...
    def permission_checker(current_user: Dict[str, Any] = Depends(get_current_user)):
        ...
        if not permission_manager.check_all_permissions(user_id, list(permissions)):
            raise HTTPException(
                status_code=403, detail=f"用戶缺少以下權限: {', '.join(permissions)}"
            )
...
def require_role(role: str):
    ...
    def role_checker(current_user: Dict[str, Any] = Depends(get_current_user)):
        ...
        if not role_manager.user_has_role(user_id, role):
            raise HTTPException(status_code=403, detail=f"用戶沒有角色: {role}")
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _is_rate_limited(self, client_ip: str) -> bool:
        ...
        if now >= minute_reset_time:
            ...
            counter["minute_reset_time"] = now.replace(second=0, microsecond=0) + timedelta(
                minutes=1
            )
        ...
        if now >= hour_reset_time:
            ...
            counter["hour_reset_time"] = now.replace(minute=0, second=0, microsecond=0) + timedelta(
                hours=1
            )
    ...
    def _record_request(self, client_ip: str):
        ...
        if client_ip not in self.request_counters:
            self.request_counters[client_ip] = {
                ...
                "hour_reset_time": now.replace(minute=0, second=0, microsecond=0)
                + timedelta(hours=1),
            }
