...
from .models import AuthAttempt, AuthStatus
...
class AuthManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.password_reset_token_expire_hours = self.config.get(
    ...
    def authenticate_user(
    ...
    ) -> Tuple[AuthStatus, Optional[Dict[str, Any]]]:
        """
        用戶認證
        ...
        try:
            ...
            refresh_token = jwt_handler.create_refresh_token(
                user_id=user_info["id"], username=username, session_id=session.session_id
            )
    ...
    def reset_password_request(self, email: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            self.reset_tokens[reset_token] = {
                ...
                + timedelta(hours=self.password_reset_token_expire_hours),
                "used": False,
            }
    ...
    def get_auth_stats(self) -> Dict[str, Any]:
        ...
        successful_attempts = sum(
            1 for attempt in self.auth_attempts if attempt.status == AuthStatus.SUCCESS
        )
        ...
        return {
            ...
            "active_verification_tokens": len(
                [t for t in self.verification_tokens.values() if not t["used"]]
            ),
        }
    ...
    def cleanup_expired_tokens(self):
        ...
        expired_reset_tokens = [
            token for token, info in self.reset_tokens.items() if now > info["expires_at"]
        ]
        ...
        expired_verification_tokens = [
            token for token, info in self.verification_tokens.items() if now > info["expires_at"]
        ]
        ...
        if expired_reset_tokens or expired_verification_tokens:
            logger.info(
                f"清理過期令牌: 重置令牌 {len(expired_reset_tokens)} 個，驗證令牌 {len(expired_verification_tokens)} 個"
            )
