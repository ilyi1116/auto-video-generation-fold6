...
from .models import Permission, ResourceType, PermissionType
...
class PermissionManager:
    ...
    def _initialize_permission_groups(self):
        """初始化基本權限組"""
        # 系統管理權限組
        self.permission_groups["system_admin"] = {
            "system:admin", "system:read", "system:config"
        }
        ...
        self.permission_groups["content_manager"] = {
            "crawler:read", "crawler:write", "crawler:create", "crawler:execute",
            "trends:read", "trends:write", "trends:analyze"
        }
        
        # 分析師權限組
        self.permission_groups["analyst"] = {
            "logs:read", "logs:analyze", "behavior:read", "behavior:analyze",
            "trends:read", "trends:analyze", "monitoring:read"
        }
        
        # 只讀用戶權限組
        self.permission_groups["readonly"] = {
            "system:read", "user:read", "crawler:read", "trends:read",
            "logs:read", "models:read", "behavior:read", "monitoring:read"
        }
    ...
    def get_user_permissions_for_resource(self, 
                                        user_id: str,
                                        resource_type: str) -> Set[str]:
        """
        獲取用戶對特定資源類型的所有權限
    ...
    def get_accessible_resources(self, 
                               user_id: str,
                               resource_type: str,
                               resource_ids: List[str]) -> List[str]:
        """
        獲取用戶可訪問的資源列表
    ...
    def filter_by_permission(self, 
                           ...
                           permission: str,
                           resource_id_field: str = "id") -> List[Dict[str, Any]]:
        """
        根據權限過濾項目列表
    ...
    def explain_permission_denial(self, 
                                user_id: str,
                                permission: str,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_grouped_permissions": sum(len(perms) for perms in self.permission_groups.values())
        }
