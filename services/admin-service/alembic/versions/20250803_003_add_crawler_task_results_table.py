...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "scraped_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_crawler_task_results_id"), "crawler_task_results", ["id"], unique=False
    )
    op.create_index(
        "ix_crawler_task_results_task_id", "crawler_task_results", ["task_id"], unique=False
    )
    op.create_index(
        "ix_crawler_task_results_run_id", "crawler_task_results", ["run_id"], unique=False
    )
