...
class PatternDetector:
    ...
    async def detect_sequential_patterns(
        self, min_support: int = 3, max_length: int = 5, time_window_hours: int = 24
        ...
        for pattern_seq, frequency in frequent_patterns.items():
            ...
            pattern = BehaviorPattern(
                ...
                conditions={"sequence": pattern_seq, "time_window_hours": time_window_hours},
                frequency=frequency,
    ...
    def _find_frequent_sequences(
        ...
        # 過濾低頻模式
        frequent_patterns = {
            pattern: count for pattern, count in pattern_counts.items() if count >= min_support
        }

        return frequent_patterns

    def _calculate_sequence_confidence(
        self, pattern_seq: Tuple[str, ...], all_sequences: List[List[str]]
    ) -> float:
        """計算序列模式置信度"""
    ...
    def _calculate_pattern_impact(
        self, pattern_seq: Tuple[str, ...], user_sequences: Dict[str, List[UserAction]]
    ) -> float:
        """計算模式影響分數"""
        # 簡化實現：基於包含該模式的用戶數量
    ...
    async def detect_temporal_patterns(self, days: int = 7) -> List[BehaviorPattern]:
        ...
        for hour, action_counts in hourly_patterns.items():
            ...
            if dominance_ratio > 0.6:  # 60%以上
                pattern_id = self._generate_pattern_id(
                    "temporal_hourly", f"{hour}_{dominant_action[0]}"
                )
        ...
        for weekday, action_counts in daily_patterns.items():
            ...
            if dominance_ratio > 0.5:  # 50%以上
                pattern_id = self._generate_pattern_id(
                    "temporal_daily", f"{weekday}_{dominant_action[0]}"
                )
    ...
    async def detect_user_journey_patterns(self, days: int = 7) -> List[BehaviorPattern]:
        ...
        for action in self.collector.actions:
            if (
                ...
                or action.action_type != ActionType.PAGE_VIEW
                or not action.page_url
            ):
                continue
            ...
            if count >= self.config["min_pattern_frequency"]:
                ...
                if confidence > 0.05:  # 5%以上的轉換
                    ...
                    pattern = BehaviorPattern(
                        ...
                        user_segments=[UserSegment.ACTIVE_USER],
                        conditions={"pattern_type": "page_transition", "transition": transition},
                        frequency=count,
                        confidence_score=confidence,
            ...
            if count >= self.config["min_pattern_frequency"]:
                ...
                if confidence > 0.03:  # 3%以上的序列
                    ...
                    pattern = BehaviorPattern(
                        ...
                        pattern_type="user_journey",
                        user_segments=[UserSegment.ACTIVE_USER],
                        conditions={"pattern_type": "page_sequence", "sequence": list(sequence)},
                        frequency=count,
                        confidence_score=confidence,
    ...
    async def detect_cohort_patterns(self, days: int = 30) -> List[BehaviorPattern]:
        ...
        for segment, action_counts in segment_behaviors.items():
            ...
            characteristic_actions = [
                (action, ratio)
                for action, ratio in action_distribution.items()
                if ratio > 0.2  # 20%以上
            ]
            ...
            if characteristic_actions:
                ...
                action_descriptions = [
                    f"{action}({ratio:.1%})" for action, ratio in characteristic_actions
                ]
                ...
                pattern = BehaviorPattern(
                    ...
                    user_segments=(
                        [UserSegment(segment)]
                        if segment in [s.value for s in UserSegment]
                        else [UserSegment.ACTIVE_USER]
                    ),
                    conditions={
                        "segment": segment,
    ...
    async def analyze_pattern_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        pattern_stats = defaultdict(
            lambda: {"count": 0, "avg_confidence": 0.0, "avg_impact": 0.0, "total_frequency": 0}
        )
        ...
        for pattern in self.detected_patterns.values():
            ...
            stats["avg_confidence"] = (
                stats["avg_confidence"] * (stats["count"] - 1) + pattern.confidence_score
            ) / stats["count"]
            stats["avg_impact"] = (
                stats["avg_impact"] * (stats["count"] - 1) + pattern.impact_score
            ) / stats["count"]
    ...
    def get_pattern_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": (
                statistics.mean([p.confidence_score for p in self.detected_patterns.values()])
