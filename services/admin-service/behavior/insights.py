...
class InsightGenerator:
    ...
    def __init__(
        ...
        self.config = {
            ...
            "priority_thresholds": {"high": 0.8, "medium": 0.5, "low": 0.0},
        }
    ...
    async def _generate_engagement_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        # 計算平均參與度
        recent_sessions = [
            s for s in self.collector.sessions.values() if s.start_time >= cutoff_time
        ]
    ...
    async def _generate_conversion_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        # 計算轉換相關指標
        recent_actions = [a for a in self.collector.actions if a.timestamp >= cutoff_time]
            ...
            insights.append(
                BehaviorInsight(
                    ...
                    data_points={
                        ...
                        "high_value_conversions": sum(
                            1 for v in conversion_values if v > avg_value * 1.5
                        ),
                    },
    ...
    async def _generate_retention_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        users_still_active = set(
            a.user_id for a in self.collector.actions if a.timestamp >= now - timedelta(days=1)
        )
            ...
            if retention_7d < 0.2:  # 20%
                insights.append(
                    BehaviorInsight(
                        ...
                        insight_type="risk",
                        priority="high",
                        affected_users=len(users_week_ago)
                        - len(users_week_ago & users_still_active),
    ...
    async def _generate_feature_usage_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        total_users = len(
            set(a.user_id for a in self.collector.actions if a.timestamp >= cutoff_time)
        )
            ...
            insights.append(
                BehaviorInsight(
                    ...
                    data_points={
                        ...
                        "avg_usage_rate": statistics.mean(
                            [f["usage_rate"] for f in low_usage_features]
                        ),
    ...
    async def _generate_performance_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        page_load_times = []
        for action in self.collector.actions:
            if action.timestamp >= cutoff_time and action.action_type == ActionType.PAGE_VIEW:
        ...
        if page_load_times:
            ...
            p95_load_time = (
                sorted(page_load_times)[int(len(page_load_times) * 0.95)]
                if len(page_load_times) > 1
                else page_load_times[0]
            )
                    ...
                    BehaviorInsight(
                        ...
                        affected_users=len(
                            set(
                                ...
                                and a.action_type == ActionType.PAGE_VIEW
                            )
                        ),
    ...
    async def _generate_trend_insights(self, days: int) -> List[BehaviorInsight]:
        ...
        current_week_actions = [
            a for a in self.collector.actions if current_week_start <= a.timestamp <= now
        ]
    ...
    async def get_insights(
        self, insight_type: Optional[str] = None, priority: Optional[str] = None, limit: int = 20
    ...
    def get_insight_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "implemented_insights": sum(
                1 for i in self.insights.values() if i.status == "implemented"
            ),
            "dismissed_insights": sum(1 for i in self.insights.values() if i.status == "dismissed"),
            "avg_confidence": (
                statistics.mean([i.confidence_level for i in self.insights.values()])
                if self.insights
                else 0
            ),
            "config": self.config,
...
def init_insight_generator(
    collector: BehaviorCollector, analyzer: BehaviorAnalyzer, pattern_detector: PatternDetector
):
    """初始化全域洞察生成器"""
