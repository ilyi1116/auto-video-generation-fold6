...
class TracingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, service_name: str = "admin-service", exclude_paths: Optional[list] = None
    ):
        super().__init__(app)
    ...
    def _extract_trace_context(self, request: Request) -> Optional[TraceContext]:
        ...
        if trace_id:
            return TraceContext(
                trace_id=trace_id, parent_span_id=parent_span_id, service_name=self.service_name
            )
...
class DatabaseTracingMiddleware:
    ...
    async def before_query(self, query: str, parameters: Dict[str, Any] = None):
        ...
        self.active_queries[query_id] = {"context": context, "start_time": time.time()}

        return query_id

    async def after_query(self, query_id: str, result_count: int = None, error: Exception = None):
        """查詢後的追蹤"""
...
class CeleryTracingMiddleware:
    ...
    @staticmethod
    def before_task_publish(sender=None, headers=None, body=None, properties=None, **kwargs):
        """任務發布前的追蹤"""
