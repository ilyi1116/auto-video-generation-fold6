...
class CRUDAIProvider(CRUDBase):
    ...
    def update_ai_provider(
        self, db: Session, *, provider_id: int, provider_in: AIProviderUpdate
    ) -> Optional[AIProvider]:
        ...
        if "name" in update_data:
            existing = (
                db.query(AIProvider)
                .filter(and_(AIProvider.name == update_data["name"], AIProvider.id != provider_id))
                .first()
            )
            if existing:
    ...
    def test_provider_connection(self, db: Session, provider_id: int) -> bool:
        ...
        try:
            # 模擬測試邏輯
            self._decrypt_api_key(provider.api_key)
            # 實際測試代碼...
            return True
...
class CRUDCrawlerConfig(CRUDBase):
    ...
    def create_crawler_config(
        self, db: Session, *, config_in: CrawlerConfigCreate
    ) -> CrawlerConfig:
        # 檢查名稱是否已存在
    ...
    def update_crawler_config(
        self, db: Session, *, config_id: int, config_in: CrawlerConfigUpdate
    ...
    def update_last_run(self, db: Session, config_id: int, run_time: datetime = None):
        ...
        if config:
            next_run_at = self._calculate_next_run(
                config.schedule_type, config.schedule_config, run_time
            )
            self.update(
                db, db_obj=config, obj_in={"last_run_at": run_time, "next_run_at": next_run_at}
            )
...
class CRUDSocialTrendConfig(CRUDBase):
    ...
    def create_trend_config(
        self, db: Session, *, config_in: SocialTrendConfigCreate
    ) -> SocialTrendConfig:
        # 檢查平台和地區組合是否已存在
        existing = (
            db.query(SocialTrendConfig)
            .filter(
                and_(
...
class CRUDTrendingKeyword(CRUDBase):
    ...
    def create_trending_keywords(
        self, db: Session, *, keywords: List[TrendingKeywordCreate]
    ...
    def get_trending_keywords(self, db: Session, *, params: TrendQueryParams) -> tuple:
        ...
        # 應用分頁和排序
        keywords = (
            query.order_by(desc(TrendingKeyword.trend_date), asc(TrendingKeyword.rank))
    ...
    def get_top_keywords(
        self, db: Session, platform: SocialPlatform, timeframe: TrendTimeframe, limit: int = 10
    ) -> List[TrendingKeyword]:
        """獲取指定平台和時間範圍的熱門關鍵字"""
        return (
            db.query(TrendingKeyword)
            .filter(
                and_(TrendingKeyword.platform == platform, TrendingKeyword.timeframe == timeframe)
            )
...
class CRUDSystemLog(CRUDBase):
    ...
    def get_dashboard_stats(self, db: Session) -> Dict[str, Any]:
        ...
        stats = {
            "total_logs_today": db.query(SystemLog)
            .filter(func.date(SystemLog.created_at) == today)
            .count(),
            "error_logs_today": db.query(SystemLog)
            .filter(
                and_(
        ...
        for i in range(24):
            ...
            count = (
                db.query(SystemLog)
                .filter(and_(SystemLog.created_at >= hour_start, SystemLog.created_at < hour_end))
                .count()
            )
...
class CRUDAdminUser(CRUDBase):
    ...
    def create_admin_user(self, db: Session, *, user_in: AdminUserCreate) -> AdminUser:
        # 檢查用戶名和郵箱是否已存在
        existing_username = (
            db.query(AdminUser).filter(AdminUser.username == user_in.username).first()
        )
...
class CRUDKeywordTrend(CRUDBase):
    ...
    def get_trends_by_platform(
        self, db: Session, platform: str, period: str = "day", limit: int = 50
    ) -> List[KeywordTrend]:
        """根據平台獲取趨勢數據"""
        return (
            db.query(KeywordTrend)
            .filter(KeywordTrend.platform == platform, KeywordTrend.period == period)
            .order_by(KeywordTrend.rank.asc())
            .limit(limit)
            .all()
        )

    def get_trends_by_period(
        self, db: Session, period: str, platforms: Optional[List[str]] = None, limit: int = 100
    ) -> List[KeywordTrend]:
        """根據時間週期獲取趨勢數據"""
        ...
        return query.order_by(KeywordTrend.platform, KeywordTrend.rank.asc()).limit(limit).all()

    def get_latest_trends(
        self, db: Session, platform: Optional[str] = None, period: str = "day", limit: int = 50
    ) -> List[KeywordTrend]:
        """獲取最新的趨勢數據"""
        ...
        return (
            query.order_by(desc(KeywordTrend.collected_at), KeywordTrend.rank.asc())
            .limit(limit)
            .all()
        )

    def get_trending_keywords_by_date_range(
        self,
        db: Session,
        start_date: datetime,
        end_date: datetime,
        ...
        return query.order_by(KeywordTrend.collected_at.desc(), KeywordTrend.rank.asc()).all()
    ...
    def get_platform_statistics(self, db: Session, days: int = 7) -> Dict[str, Any]:
        ...
        platform_stats = (
            ...
            .filter(KeywordTrend.collected_at >= since_date)
            .group_by(KeywordTrend.platform)
            .all()
        )
    ...
    def search_keywords(
        self, db: Session, search_term: str, platforms: Optional[List[str]] = None, limit: int = 100
    ) -> List[KeywordTrend]:
        """搜尋關鍵字"""
        query = db.query(KeywordTrend).filter(KeywordTrend.keyword.ilike(f"%{search_term}%"))

        if platforms:
            query = query.filter(KeywordTrend.platform.in_(platforms))

        return (
            query.order_by(KeywordTrend.rank.asc(), desc(KeywordTrend.collected_at))
            .limit(limit)
            .all()
        )

    def get_top_keywords_by_period(
        self, db: Session, period: str, top_n: int = 10
        ...
        latest_collections = (
            ...
            .filter(KeywordTrend.period == period)
            .group_by(KeywordTrend.platform)
            .subquery()
        )
        ...
        result = {}
        platforms = (
            db.query(KeywordTrend.platform).filter(KeywordTrend.period == period).distinct().all()
        )

        for (platform,) in platforms:
            latest_date = (
                db.query(func.max(KeywordTrend.collected_at))
                .filter(KeywordTrend.platform == platform, KeywordTrend.period == period)
                .scalar()
            )
...
class CRUDCrawlerTask(CRUDBase):
    ...
    def update_crawler_task(
        self, db: Session, *, task_id: int, task_in: CrawlerTaskUpdate
        ...
        if "task_name" in update_data:
            existing = (
                ...
                .filter(
                    and_(
                        CrawlerTask.task_name == update_data["task_name"], CrawlerTask.id != task_id
                    )
                )
                .first()
            )
    ...
    def get_tasks_by_schedule_type(self, db: Session, schedule_type: str) -> List[CrawlerTask]:
        ...
        return (
            db.query(CrawlerTask)
            .filter(and_(CrawlerTask.schedule_type == schedule_type, CrawlerTask.is_active == True))
            .all()
        )
    ...
    def get_task_statistics(self, db: Session) -> Dict[str, Any]:
        ...
        # 按排程類型統計
        schedule_stats = (
            db.query(CrawlerTask.schedule_type, func.count(CrawlerTask.id).label("count"))
            .group_by(CrawlerTask.schedule_type)
            .all()
        )
