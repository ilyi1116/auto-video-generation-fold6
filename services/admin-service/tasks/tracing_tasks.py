...
from ..tracing import trace_collector, trace_analyzer
...
def analyze_traces_task(self, analysis_params: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"開始分析追蹤資料: {analysis_type}, 服務: {service_name}, 時間範圍: {hours}小時")
        
        # 執行不同類型的分析
        if analysis_type == "performance":
            result = asyncio.run(
                trace_analyzer.analyze_performance(
            ...
            result = asyncio.run(
                trace_analyzer.analyze_errors(hours=hours)
            )
        elif analysis_type == "services":
            result = asyncio.run(
                trace_analyzer.analyze_services(hours=hours)
            )
        elif analysis_type == "trends":
            interval_minutes = analysis_params.get("interval_minutes", 60)
            result = asyncio.run(
                trace_analyzer.analyze_trends(
                    hours=hours,
...
def health_check_task(self, service_name: str = None):
    ...
    try:
        ...
        health_score = asyncio.run(
            trace_analyzer.get_health_score(
                service_name=service_name,
                hours=1
            )
        )
        ...
        # 獲取慢操作
        slow_operations = asyncio.run(
            trace_analyzer.get_slow_operations(
                limit=5,
                hours=24
            )
        )
    ...
    try:
        ...
        report = {
            ...
            "executive_summary": {
                ...
                "avg_response_time": performance_analysis.get("performance_metrics", {}).get("avg_duration_ms", 0)
            },
