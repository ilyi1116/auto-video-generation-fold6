...
from ..logging_system import AuditLogger, cleanup_old_logs, log_analyzer
...
def cleanup_old_logs_task(self, days: int = 30):
    ...
    try:
        ...
        return {"success": True, "days": days, "cleanup_time": datetime.utcnow().isoformat()}
...
def system_health_check(self):
    ...
    try:
        ...
        try:
            ...
            health_data["celery_status"] = {"healthy": celery_healthy, "message": celery_message}
        except Exception as e:
            health_data["celery_status"] = {"healthy": False, "message": f"檢查失敗: {str(e)}"}
        ...
        health_data["status"] = (
            "healthy" if health_score >= 70 else "warning" if health_score >= 40 else "critical"
        )
...
def backup_database(self):
    ...
    try:
        ...
        f"{backup_dir}/admin_db_backup_{timestamp}.sql"
        ...
        # SQLite 示例:
        db_path = os.getenv(
            "DATABASE_PATH", "/data/data/com.termux/files/home/myProject/data/admin.db"
        )
        ...
        backup_size = (
            os.path.getsize(f"{backup_dir}/admin_db_backup_{timestamp}.db")
            if os.path.exists(f"{backup_dir}/admin_db_backup_{timestamp}.db")
            else 0
        )
...
def update_system_stats(self):
    ...
    try:
        ...
        stats["logs"] = {
            ...
            "today": db.query(SystemLog).filter(func.date(SystemLog.created_at) == today).count(),
            "errors_today": db.query(SystemLog)
            .filter(
                func.date(SystemLog.created_at) == today, SystemLog.level.in_(["error", "critical"])
            )
            .count(),
        }
        ...
        stats["trend_configs"] = {
            ...
            "active": db.query(SocialTrendConfig)
            .filter(SocialTrendConfig.is_active == True)
            .count(),
        }
        ...
        stats["trending_keywords"] = {
            ...
            "today": db.query(TrendingKeyword)
...
def schedule_maintenance_task(task_name: str, delay_seconds: int = 0, **kwargs):
    ...
    if delay_seconds > 0:
        return task.apply_async(kwargs=kwargs, countdown=delay_seconds, queue="maintenance")
    else:
