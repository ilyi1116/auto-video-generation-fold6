...
def health_monitoring_task(self):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"健康監控完成: {result['overall_status']}, {result['active_alerts_count']} 個活躍告警"
            )
...
def metrics_aggregation_task(self, hours: int = 1):
    ...
    try:
        ...
        try:
            ...
            healthy_count = sum(
                1 for record in filtered_history if record.get("overall_status") == "healthy"
            )
            warning_count = sum(
                1 for record in filtered_history if record.get("overall_status") == "warning"
            )
            critical_count = sum(
                1 for record in filtered_history if record.get("overall_status") == "critical"
            )
            ...
            avg_execution_time = (
                total_execution_time / execution_count if execution_count > 0 else 0
            )
            ...
            logger.info(
                f"指標聚合完成: 處理 {total_records} 條記錄, 可用性 {aggregated_data['availability_percentage']:.2f}%"
            )
