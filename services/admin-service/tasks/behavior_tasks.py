...
def analyze_user_behavior_task(self, user_id: str, days: int = 30):
    ...
    try:
        ...
        # 執行用戶行為分析
        analysis = asyncio.run(behavior_analyzer.analyze_user_behavior(user_id, days))
...
def detect_behavior_patterns_task(self, pattern_type: str = "all", days: int = 7):
    ...
    try:
        ...
        if pattern_type in ["temporal", "all"]:
            temporal_patterns = asyncio.run(pattern_detector.detect_temporal_patterns(days=days))
            patterns.extend(temporal_patterns)

        if pattern_type in ["journey", "all"]:
            journey_patterns = asyncio.run(pattern_detector.detect_user_journey_patterns(days=days))
            patterns.extend(journey_patterns)

        if pattern_type in ["cohort", "all"]:
            cohort_patterns = asyncio.run(pattern_detector.detect_cohort_patterns(days=days))
...
def generate_behavior_insights_task(self, days: int = 7):
    ...
    try:
        ...
        # 生成洞察
        insights = asyncio.run(insight_generator.generate_insights(days))
...
def analyze_page_performance_task(self, page_url: str = None, hours: int = 24):
    ...
    try:
        ...
        # 執行頁面分析
        analysis = asyncio.run(
            behavior_analyzer.analyze_page_performance(page_url=page_url, hours=hours)
        )
...
def detect_behavior_anomalies_task(self, hours: int = 24):
    ...
    try:
        ...
        # 執行異常檢測
        anomalies = asyncio.run(behavior_analyzer.detect_anomalies(hours))
...
def export_behavior_data_task(
    self, user_id: str = None, start_date: str = None, end_date: str = None, format: str = "json"
):
    """
    導出行為數據任務
    ...
    try:
        ...
        data = asyncio.run(
            behavior_collector.export_data(
                user_id=user_id, start_date=start_dt, end_date=end_dt, format=format
            )
        )
...
def behavior_health_check_task(self):
    ...
    try:
        ...
        health_report = {
            ...
            "components": {
                "behavior_collector": {"status": "healthy", "stats": collector_stats},
                "behavior_analyzer": {"status": "healthy", "stats": analyzer_stats},
                "pattern_detector": {"status": "healthy", "stats": pattern_stats},
                "insight_generator": {"status": "healthy", "stats": insight_stats},
            },
...
def daily_behavior_analysis_task(self):
    ...
    try:
        ...
        try:
            ...
            results["patterns"] = {"sequential_patterns": len(patterns_result), "status": "success"}
        except Exception as e:
        ...
        # 2. 生成洞察
        try:
            insights_result = asyncio.run(insight_generator.generate_insights(days=1))
            results["insights"] = {
        ...
        # 3. 異常檢測
        try:
            anomalies_result = asyncio.run(behavior_analyzer.detect_anomalies(hours=24))
            results["anomalies"] = {
                "anomalies_detected": len(anomalies_result),
                "critical_anomalies": len(
                    [a for a in anomalies_result if a.get("severity") == "critical"]
                ),
                "status": "success",
            }
        except Exception as e:
