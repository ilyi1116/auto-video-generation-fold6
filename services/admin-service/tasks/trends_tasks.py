...
class TrendsTask(TaskRetryMixin):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        # 記錄失敗日誌
        asyncio.create_task(
            AuditLogger.log_error(error=exc, context=f"Celery趨勢任務失敗: {task_id}")
        )
...
def cleanup_old_trends(self, days: int = 90):
    ...
    try:
        ...
        deleted_old_count = (
            db.query(TrendingKeyword).filter(TrendingKeyword.trend_date < cutoff_date).delete()
        )

        # 清理舊的 keyword_trends 數據
        deleted_new_count = (
            db.query(KeywordTrend).filter(KeywordTrend.collected_at < cutoff_date).delete()
        )
        ...
        logger.info(
            f"清理了 {total_deleted} 條超過 {days} 天的趨勢數據 (舊格式: {deleted_old_count}, 新格式: {deleted_new_count})"
        )
...
def analyze_trend_patterns(self):
    ...
    try:
        ...
        top_keywords = (
            ...
            .filter(TrendingKeyword.trend_date >= last_week)
        ...
        trending_up = (
            db.query(
                ...
                func.avg(
                    func.case(
                        [
                            (
                                TrendingKeyword.trend_date >= datetime.utcnow() - timedelta(days=1),
            ...
            .filter(TrendingKeyword.trend_date >= last_week)
            .group_by(TrendingKeyword.keyword)
            .having(func.count(TrendingKeyword.id) >= 5)  # 至少出現5次
            .all()
        )
...
def generate_trends_report(self):
    ...
    from ..models import SocialTrendConfig, TrendingKeyword
    ...
    try:
        ...
        last_24h_trends = (
            db.query(TrendingKeyword).filter(TrendingKeyword.trend_date >= last_24h).count()
        )

        # 活躍配置統計
        active_configs = (
            db.query(SocialTrendConfig).filter(SocialTrendConfig.is_active == True).count()
        )

        # 各平台今日趨勢統計
        platform_today_stats = (
            db.query(TrendingKeyword.platform, func.count(TrendingKeyword.id).label("count"))
            .filter(func.date(TrendingKeyword.trend_date) == today)
            .group_by(TrendingKeyword.platform)
            .all()
        )
        ...
        for platform in SocialPlatform:
            top_keywords = (
                ...
                .order_by(TrendingKeyword.rank.asc())
                .limit(5)
                .all()
            )

            today_top_keywords[platform.value] = [
                {"keyword": kw.keyword, "rank": kw.rank, "score": kw.score} for kw in top_keywords
            ]
        ...
        return {
            ...
            "platform_stats": [
                {"platform": stat.platform, "today_count": stat.count}
...
def validate_trend_configs(self):
    ...
    try:
        configs = db.query(SocialTrendConfig).filter(SocialTrendConfig.is_active == True).all()
...
# 手動觸發任務的工具函數
def schedule_trends_collection(
    platform: str = None, region: str = "global", delay_seconds: int = 0
):
    """排程趨勢收集"""
    if platform:
    ...
    if delay_seconds > 0:
        return task.apply_async(args=args, countdown=delay_seconds, queue="trends")
    else:
...
# 新版本 API 的便利函數
def schedule_keyword_trends_collection(
    platforms: List[str] = None, period: str = "day", delay_seconds: int = 0
):
    """排程關鍵字趨勢收集 (新版本)"""
    if delay_seconds > 0:
        return collect_keyword_trends_new.apply_async(
            args=[platforms, period], countdown=delay_seconds, queue="trends"
        )
    else:
