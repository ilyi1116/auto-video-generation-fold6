...
from ..auth import (
    auth_manager, jwt_handler, password_manager, role_manager,
    session_manager, permission_manager, init_auth_system
)
...
def role_audit_task(self):
    ...
    try:
        ...
        # 審計未使用的角色
        unused_roles = [
            role_name for role_name, stats in role_usage.items()
    ...
    try:
        ...
        if results["active_users_count"] == 0:
            results["recommendations"].append({
                ...
                "description": "當前沒有活躍用戶，可能需要檢查系統狀態"
            })
        elif results["active_users_count"] > 100:
            results["recommendations"].append({
                "type": "high_user_activity",
                "description": "用戶活動量很高，建議監控系統性能"
            })
...
def auth_health_check_task(self):
    ...
    try:
        ...
        try:
            ...
            health_report["components"]["auth_manager"] = {
                "status": "healthy",
                "stats": auth_stats
            }
        except Exception as e:
        ...
        try:
            role_stats = role_manager.get_stats()
            health_report["components"]["role_manager"] = {
                "status": "healthy",
                "stats": role_stats
            }
        except Exception as e:
        ...
        try:
            jwt_stats = jwt_handler.get_stats()
            health_report["components"]["jwt_handler"] = {
                "status": "healthy",
                "stats": jwt_stats
            }
        ...
        if health_report["health_issues"]:
            health_report["overall_status"] = "warning" if len(health_report["health_issues"]) <= 2 else "unhealthy"
...
def generate_security_report_task(self, days: int = 7):
    ...
    try:
        ...
        success_rate = (
            ...
            if report["summary"]["total_auth_attempts"] > 0 else 0
        )
