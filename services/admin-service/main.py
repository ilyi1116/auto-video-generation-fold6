...
from .crud import (
    ...
    crud_crawler_task,
    crud_keyword_trend,
    crud_social_trend_config,
    crud_system_log,
    crud_trending_keyword,
)
...
from .schemas import (
    ...
    LogQueryParams,
    PaginatedResponse,
    SocialTrendConfigCreate,
    SocialTrendConfigResponse,
    SocialTrendConfigUpdate,
    SystemLogResponse,
...
async def get_current_user(
    ...
    if not user.is_active:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="用戶已被禁用")
...
def require_permission(permission: str):
    ...
    def permission_checker(
        current_user: AdminUser = Depends(get_current_user), request: Request = None
    ):
        from .security import PermissionChecker

        has_permission = PermissionChecker.check_permission(
            current_user.role, current_user.permissions, permission
        )
        ...
        if not has_permission:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"缺少必要權限: {permission}"
            )
...
async def security_middleware(request: Request, call_next):
    ...
    if any(request.url.path.startswith(path) for path in sensitive_paths):
        if not check_ip_whitelist(request):
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="訪問被拒絕：IP 不在白名單中"
            )
    ...
    if not rate_limiter.is_allowed(client_ip):
        ...
        await AuditLogger.log_security_event(
            "rate_limit_exceeded", "warning", f"IP {client_ip} 超過速率限制", ip_address=client_ip
        )
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="請求過於頻繁，請稍後再試"
        )
    ...
    await AuditLogger.log_api_request(
        request=request, response_status=response.status_code, duration_ms=int(process_time)
    )
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content=ErrorResponse(
            message="內部伺服器錯誤", errors=[str(exc)], error_code="INTERNAL_ERROR"
        ).dict(),
    )
...
async def get_current_user_info(current_user: AdminUser = Depends(get_current_user)):
    """獲取當前用戶信息"""
    return APIResponse(data=AdminUserResponse.from_orm(current_user))
...
async def create_ai_provider(
    ...
    try:
        ...
        return APIResponse(
            message="AI Provider 創建成功", data=AIProviderResponse.from_orm(provider)
        )
...
async def update_ai_provider(
    ...
    try:
        ...
        return APIResponse(
            message="AI Provider 更新成功", data=AIProviderResponse.from_orm(provider)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def test_ai_provider(
    ...
    return APIResponse(
        message="連接測試完成", data={"success": success, "provider_id": provider_id}
    )
...
async def create_crawler_config(
    ...
    try:
        ...
        return APIResponse(message="爬蟲配置創建成功", data=CrawlerConfigResponse.from_orm(config))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def update_crawler_config(
    ...
    try:
        ...
        return APIResponse(message="爬蟲配置更新成功", data=CrawlerConfigResponse.from_orm(config))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def run_crawler_manually(
    ...
    try:
        ...
        return APIResponse(message="爬蟲運行完成", data=result)
...
async def get_crawler_results(
    ...
    results = (
        ...
        .order_by(CrawlerResult.scraped_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def create_crawler_task(
    ...
    try:
        ...
        return APIResponse(message="爬蟲任務創建成功", data=task_response)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_crawler_task(
    task_id: int, current_user: AdminUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    """獲取指定爬蟲任務"""
...
async def update_crawler_task(
    ...
    """更新爬蟲任務"""
    try:
        task = crud_crawler_task.update_crawler_task(db, task_id=task_id, task_in=task_in)
        if not task:
            raise HTTPException(status_code=404, detail="爬蟲任務不存在")
        ...
        return APIResponse(message="爬蟲任務更新成功", data=task_response)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def run_crawler_task_manually(
    ...
    try:
        ...
        return APIResponse(message="爬蟲任務已排程執行", data=result)
    except Exception as e:
...
async def get_crawler_task_results(
    ...
    # 查詢結果
    results = (
        db.query(CrawlerTaskResult)
        .filter(CrawlerTaskResult.task_id == task_id)
        .order_by(CrawlerTaskResult.scraped_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
    ...
    for result in results:
        try:
            matched_keywords = (
                json.loads(result.matched_keywords) if result.matched_keywords else []
            )
    ...
    # 獲取總數
    total = db.query(CrawlerTaskResult).filter(CrawlerTaskResult.task_id == task_id).count()
...
async def get_crawler_task_statistics(
    current_user: AdminUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    """獲取爬蟲任務統計信息"""
...
async def create_social_trend_config(
    ...
    try:
        ...
        return APIResponse(
            message="社交媒體趨勢配置創建成功", data=SocialTrendConfigResponse.from_orm(config)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def update_social_trend_config(
    ...
    return APIResponse(
        message="社交媒體趨勢配置更新成功", data=SocialTrendConfigResponse.from_orm(updated_config)
    )
...
async def collect_trends_manually(
    ...
    """手動收集社交媒體趨勢"""
    from .social_trends import SocialPlatform, SocialTrendsManager
    ...
    try:
        ...
        return APIResponse(message="趨勢收集完成", data=result)
    except ValueError:
...
async def get_top_trending_keywords(
    ...
    try:
        ...
        keywords = crud_trending_keyword.get_top_keywords(db, platform_enum, timeframe_enum, limit)

        return APIResponse(data=[TrendingKeywordResponse.from_orm(k) for k in keywords])
    except ValueError as e:
...
async def get_trending_keywords_stats(
    ...
    today_count = (
        db.query(TrendingKeyword).filter(func.date(TrendingKeyword.trend_date) == today).count()
    )

    return APIResponse(
        data={
            "platform_stats": [
                {"platform": stat.platform, "count": stat.count, "last_update": stat.last_update}
...
async def cleanup_old_logs(
    ...
    try:
        ...
        return APIResponse(message=f"成功清理超過 {days} 天的舊日誌")
    except Exception as e:
...
async def create_admin_user(
    ...
    try:
        ...
        return APIResponse(message="管理員用戶創建成功", data=AdminUserResponse.from_orm(user))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def health_check():
    ...
    try:
        ...
        is_healthy = (
            db_status == "healthy"
            and celery_healthy
            and system_info["cpu_usage"] < 90
            and system_info["memory_usage"] < 90
            and system_info["disk_usage"] < 95
        )
...
async def trigger_trends_collection(
    ...
    try:
        ...
        return APIResponse(data=result, message="趨勢數據收集任務已觸發")
    except Exception as e:
...
async def get_trends_by_date_range(
    ...
    try:
        trends = crud_keyword_trend.get_trending_keywords_by_date_range(
            db, start_date=start_date, end_date=end_date, platform=platform, period=period
        )
...
async def get_system_tasks(
    ...
    try:
        ...
        return APIResponse(success=False, message="獲取任務狀態失敗", errors=[str(e)])
...
@require_permission("system:dashboard")
async def get_component_health(current_user: AdminUser = Depends(get_current_user)):
    """獲取各組件健康狀態"""
    try:
        status = health_monitor.get_health_status()
...
@require_permission("system:dashboard")
async def get_active_alerts(current_user: AdminUser = Depends(get_current_user)):
    """獲取活躍告警"""
    try:
...
@require_permission("system:dashboard")
async def get_metrics_history(hours: int = 24, current_user: AdminUser = Depends(get_current_user)):
    """獲取指標歷史數據"""
    try:
        if hours > 168:  # 限制最多7天
            hours = 168
...
async def trigger_health_check(current_user: AdminUser = Depends(get_current_user)):
    """手動觸發健康檢查"""
    try:
...
async def update_thresholds(thresholds: dict, current_user: AdminUser = Depends(get_current_user)):
    """更新監控閾值"""
    try:
        # 驗證閾值格式
        valid_metrics = [
        ...
        return APIResponse(message="閾值更新成功", data=health_monitor.thresholds)
...
@require_permission("system:dashboard")
async def get_thresholds(current_user: AdminUser = Depends(get_current_user)):
    """獲取當前監控閾值"""
    try:
...
async def start_monitoring(current_user: AdminUser = Depends(get_current_user)):
    """啟動監控"""
    try:
        if health_monitor.running:
            return APIResponse(success=False, message="監控已在運行中")
...
async def stop_monitoring(current_user: AdminUser = Depends(get_current_user)):
    """停止監控"""
    try:
...
@require_permission("system:dashboard")
async def get_rate_limit_stats(current_user: AdminUser = Depends(get_current_user)):
    """獲取限流統計"""
    try:
...
@require_permission("system:dashboard")
async def get_threat_analysis(hours: int = 24, current_user: AdminUser = Depends(get_current_user)):
    """獲取威脅分析"""
    try:
        ...
        try:
            ...
            threat_stats = {
                ...
                "rate_limit_violations": len(
                    [log for log in security_logs if log.action == "rate_limit_exceeded"]
                ),
                "invalid_tokens": len(
                    [log for log in security_logs if log.action == "invalid_token"]
                ),
                "failed_logins": len(
                    [log for log in security_logs if log.action == "login_failed"]
                ),
            ...
            threat_stats["top_threat_ips"] = [{"ip": ip, "count": count} for ip, count in top_ips]
...
async def add_to_blacklist(request_data: dict, current_user: AdminUser = Depends(get_current_user)):
    """添加 IP 到黑名單"""
    try:
        ...
        return APIResponse(
            ...
            data={"ip_address": ip_address, "duration_hours": duration_hours},
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def remove_from_blacklist(
    ip_address: str, current_user: AdminUser = Depends(get_current_user)
):
    """從黑名單移除 IP"""
    try:
        ...
        return APIResponse(
            message=f"已將 IP {ip_address} 從黑名單移除", data={"ip_address": ip_address}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@require_permission("system:dashboard")
async def get_blacklist(current_user: AdminUser = Depends(get_current_user)):
    """獲取黑名單 IP"""
    try:
...
@require_permission("system:dashboard")
async def test_rate_limit(request: Request, current_user: AdminUser = Depends(get_current_user)):
    """測試限流功能"""
    try:
        # 檢查當前請求的限流狀態
        ...
        if limit_result and not limit_result.allowed:
            ...
            return APIResponse(
                message="請求通過限流檢查",
                data={"allowed": True, "message": "當前請求未觸發任何限流規則"},
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
