...
class WebCrawler:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            connector=connector, timeout=timeout, headers={"User-Agent": self.user_agents[0]}
        )
        return self
    ...
    async def crawl_config(self, config: CrawlerConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"爬蟲配置 {config.name} 執行完成，爬取 {pages_crawled} 頁，耗時 {duration:.2f} 秒"
            )
    ...
    async def _crawl_single_page(
        self, config: CrawlerConfig, url: str, run_id: str, page_num: int
                    ...
                    keywords_found.append(
                        {
                            "keyword": keyword,
                            "count": len(matches),
                            "positions": [m.start() for m in pattern.finditer(clean_text)][
                                :10
                            ],  # 最多記錄10個位置
                        }
                    )
    ...
    def _log_crawler_error(self, config_id: int, run_id: str, error_message: str):
        ...
        try:
            ...
            log_data = SystemLogCreate(
                ...
                details={"config_id": config_id, "run_id": run_id, "error": error_message},
            )
    ...
    async def crawl_task(self, task_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"爬蟲任務 {task_name} 執行完成，爬取 {pages_crawled} 頁，耗時 {duration:.2f} 秒"
            )
    ...
    async def _crawl_task_page(
        self, task_config: Dict[str, Any], url: str, run_id: str, page_num: int
                ...
                if response.status == 200:
                    ...
                    for keyword in keywords:
                        if (
                            ...
                            or keyword.lower() in title_text.lower()
                        ):
    ...
    def _save_task_result(
        self, task_config: Dict[str, Any], result_data: Dict[str, Any], run_id: str
    ):
        """保存爬蟲任務結果到資料庫"""
        try:
            db = SessionLocal()
            ...
            if task_id:
                ...
                matched_keywords_json = json.dumps(
                    result_data.get("matched_keywords", []), ensure_ascii=False
                )
