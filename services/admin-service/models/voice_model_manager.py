...
class VoiceModelManager:
    ...
    async def start_training(
        self, model_id: str, training_config: TrainingConfig, created_by: str
    ) -> bool:
        """
        開始模型訓練
    ...
    async def update_training_progress(
        self, model_id: str, epoch: int, training_loss: float, validation_loss: float
    ):
        """
        更新訓練進度
    ...
    async def deploy_model(
        self, model_id: str, version: str, deploy_config: DeploymentConfig, deployed_by: str
    ) -> str:
        """
        部署模型
        ...
        try:
            ...
            async with get_db() as db:
                deployment = (
                    await db.query(ModelDeployment)
                    .filter(ModelDeployment.deployment_id == deployment_id)
                    .first()
                )
    ...
    async def get_model_list(
        ...
        try:
            async with get_db() as db:
                ...
                models = (
                    await query.order_by(VoiceModel.created_at.desc())
                    .offset(offset)
                    .limit(limit)
                    .all()
                )
    ...
    async def get_model_details(self, model_id: str) -> Optional[Dict]:
        ...
        try:
            async with get_db() as db:
                ...
                deployments = (
                    await db.query(ModelDeployment)
                    .filter(ModelDeployment.model_id == model_id)
        ...
        try:
            async with get_db() as db:
                ...
                active_deployments = (
                    await db.query(ModelDeployment)
                    .filter(
                        ModelDeployment.model_id == model_id, ModelDeployment.status == "running"
                    )
                    .count()
                )
                ...
                await db.query(ModelDeployment).filter(
    ...
    async def _create_version_record(
        self, model_id: str, version: str, model_path: str, metrics: ModelPerformanceMetrics
    ):
        """創建版本記錄"""
        try:
            async with get_db() as db:
