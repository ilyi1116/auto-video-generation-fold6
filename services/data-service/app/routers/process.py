...
async def start_processing(
    ...
    query = voice_files.select().where(
        (voice_files.c.id == file_id) & 
        (voice_files.c.user_id == user_id)
    )
    ...
    try:
        ...
        if job_type == JobType.PREPROCESSING:
            ...
            raise HTTPException(
                status_code=501,
                detail="Training jobs not yet implemented"
            )
            ...
            raise HTTPException(
                status_code=501,
                detail="Inference jobs not yet implemented"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to start processing job"
        )
