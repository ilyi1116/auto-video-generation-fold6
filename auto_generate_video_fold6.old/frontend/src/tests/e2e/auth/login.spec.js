...
test.describe('用戶登入功能', () => {
  ...
  test('應該在手機版面正確顯示', async ({ page }) => {
    ...
    // 驗證元素大小適合觸控操作
    const buttonBoundingBox = await page.locator('[data-testid="login-button"]').boundingBox();
  ...
  test('應該處理網路錯誤', async ({ page }) => {
    ...
    // 驗證網路錯誤訊息
    await expect(page.locator('[data-testid="error-message"]')).toBeVisible();
    await expect(page.locator('[data-testid="error-message"]')).toContainText('網路連接失敗');
  });
