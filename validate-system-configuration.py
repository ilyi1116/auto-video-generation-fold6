...
class SystemConfigurationValidator:
    ...
    def validate_docker_configuration(self) -> bool:
        ...
        docker_files = ["docker-compose.yml", "docker-compose.unified.yml"]
        ...
        if issues:
            self.log_result("Docker 配置", False, f"發現 {len(issues)} 個問題", {"issues": issues})
            return False
    ...
    def validate_shared_libraries(self) -> bool:
        ...
        shared_files = ["config.py", "__init__.py"]
    ...
    def validate_monitoring_configuration(self) -> bool:
        ...
        if issues:
            self.log_result("監控配置", False, f"發現 {len(issues)} 個問題", {"issues": issues})
            return False
